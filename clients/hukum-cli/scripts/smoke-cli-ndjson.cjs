...
//
//   - `hukum --version` → emits one `result` ok with a string payload.
//   - `hukum host status --json` against an empty HUKUM_HOME →
...
//     hold.
//   - `hukum host foo-does-not-exist --json` → exercises the
...
function smokeVersion() {
  ...
  if (trimmed.length === 0) {
    throw new Error("`hukum --version --json` produced no stdout");
  }
...
function smokeWhoamiNoCredentials() {
  ...
  const tmpHome = fs.mkdtempSync(
    path.join(os.tmpdir(), "hukum-ndjson-whoami-"),
  );
  try {
...
function smokeConfigEnvListEmpty() {
  ...
  // for Desktop's `runHukumCliJson` call site.
  const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "hukum-ndjson-cfg-"));
  try {
...
function smokeHostStatusEmptyHome() {
  // Pin HUKUM_HOME to a freshly-created empty directory so the command
  ...
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "hukum-ndjson-"));
  try {

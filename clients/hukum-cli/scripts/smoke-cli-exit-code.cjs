...
function resolveIterations() {
  const raw = process.env.HUKUM_CLI_EXIT_ITERATIONS;
  if (typeof raw !== "string" || raw.length === 0) return DEFAULT_ITERATIONS;
  const parsed = Number.parseInt(raw, 10);
  if (!Number.isInteger(parsed) || parsed < 1) {
    fail(
      `HUKUM_CLI_EXIT_ITERATIONS must be a positive integer, got '${raw}'`,
    );
  }
  return parsed;
}
...
// clients/desktop/src/electron-main/cli/hukum-cli.ts - this gate has to

...
//
//   1. `dist-sea/hukum[.exe]` exists (`build:sea` ran first).
...
//   3. The expected commander surface is reachable (`hukum --version`
...
function main() {
  if (!fs.existsSync(binaryPath)) {
    fail(
      `${binaryPath} not found. Run \`bun run --filter @hukum-clients/hukum-cli build:sea\` first.`,
    );
  }
  ...
  if (result.status !== 0) {
    fail(
      `\`hukum --version\` exited with status=${result.status}, stderr=${result.stderr}`,
    );
  }
  const out = (result.stdout || "").trim();
  if (out.length === 0) {
    fail("`hukum --version` produced no stdout");
  }
  // Regression guard for ticket:e86b8372-…/284b9132-… - the pre-fix
  // entrypoint advertised a hardcoded `0.0.0` regardless of what
  // `HUKUM_CLI_VERSION` injected. The local-fallback sentinel is
  // `0.0.0-local`, so we only refuse the bare `0.0.0` shape here.
  if (out === "0.0.0") {
    fail(
      `\`hukum --version\` reported the pre-fix placeholder '0.0.0'; the SEA build is not consuming HUKUM_CLI_VERSION`,
    );
  }
  ...
  const expected = process.env.HUKUM_CLI_VERSION_EXPECT;
  if (typeof expected === "string" && expected.length > 0 && out !== expected) {
    fail(
      `\`hukum --version\` reported '${out}' but the test harness expected '${expected}' (HUKUM_CLI_VERSION_EXPECT)`,
    );
  }
  if (typeof expected === "string" && expected.length > 0) {
    const hostileEnv = {
      ...cleanEnv,
      HUKUM_CLI_VERSION: "0.0.0-local",
    };
    const hostileResult = runVersionProbe(hostileEnv);
    if (hostileResult.status !== 0) {
      fail(
        `hostile-env \`hukum --version\` exited with status=${hostileResult.status}, stderr=${hostileResult.stderr}`,
      );
    }
    const hostileOut = (hostileResult.stdout || "").trim();
    if (hostileOut !== expected) {
      fail(
        `hostile-env \`hukum --version\` reported '${hostileOut}' but expected baked version '${expected}'`,
      );
    }
  }

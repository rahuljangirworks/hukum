...
// Production SEA build for the Hukum CLI. Produces a single, native
// executable per host platform/arch with no `node` / `bun` runtime
// dependency. Output: `dist-sea/hukum[.exe]`.
//
...
const outputBinary = path.join(distDir, exeName("hukum"));
...
async function main() {
  ...
  // Inject the released CLI version into the bundle so `hukum --version`
  // reports the artifact's actual version rather than the source-tree
  // placeholder. CI release workflows set `HUKUM_CLI_VERSION` to the
  ...
  // clients/hukum-cli/src/index.ts - its `LOCAL_CLI_VERSION`
  ...
  // (an aliased `process.env`), NOT a literal `process.env.HUKUM_CLI_VERSION`
  ...
  // `process.env.HUKUM_CLI_VERSION` access elsewhere in the bundle.
  const cliVersion =
  ...
  const cliSentryDsn =
    typeof process.env.HUKUM_CLI_SENTRY_DSN === "string"
      ? process.env.HUKUM_CLI_SENTRY_DSN
      : "";

  // The host trusted pubkeys (the registry signature trust root) are baked
  // into `src/config.ts` by the deploy step
  // (scripts/set-deploy-target.cjs, from HUKUM_EMBEDDED_HOST_PUBKEYS)
  ...
  const defines = {
    ...
    "process.env.HUKUM_CLI_SENTRY_DSN": JSON.stringify(cliSentryDsn),
  };
  ...
  const sentryPlugins = (() => {
    ...
    return [
      sentryEsbuildPlugin({
        ...
        project: process.env.SENTRY_PROJECT || "hukum-cli",
      }),
    ];
  })();

...
// Stamp the release-time values onto `clients/hukum-cli/src/config.ts` for a
...
// RELEASE_REPO).
const DEFAULT_RELEASE_REPO = "hukumai/hukum";
...
const supportedHostVersion = parseSupportedHostVersion(
  process.argv,
  process.env.HUKUM_SUPPORTED_HOST_VERSION,
);
const releaseRepo = parseReleaseRepo(
  process.env.HUKUM_RELEASE_REPO ?? process.env.RELEASE_REPO,
);
...
if (
  ...
  console.error(
    "[set-deploy-target] HUKUM_SUPPORTED_HOST_VERSION or --supported-host-version=<version> is required for production CLI builds (the released CLI must install a pinned host version by default). Pass --allow-unpinned-host only for local dogfood installs that side-load an unsigned host.",
  );
  process.exit(2);
}

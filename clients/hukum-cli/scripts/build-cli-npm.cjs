...
// npm-publishable JS build for the Hukum CLI.
//
...
// `bin` entry. It is what backs the public `@hukumai/cli` npm package.
//
// Why bundle instead of `tsc` → dist: the CLI imports
//   - `@hukum-clients/shared/*` — a PRIVATE, unpublished workspace
//     package (cannot be an npm dependency), and
//   - `@hukum/protocol/*` — published as *extensionless, bundler-only*
...
//
// Output (gitignored):
//   dist-npm/hukum        single bundled, extensionless CJS file
//                           + `#!/usr/bin/env node` (the basename MUST be
//                           `hukum` - see BUNDLE_BASENAME below)
//   dist-npm/package.json   GENERATED publish manifest (name: @hukumai/cli)
//
// The published name (`@hukumai/cli`) is decoupled from the workspace
// name (`@hukum-clients/hukum-cli`, a build label imported nowhere)
...
// The bundle MUST be named `hukum` (no extension). The CLI entry's
// script-mode guard `isHukumCliEntrypoint(process.argv[1])` only runs
// `parseAsync` when argv[1] basename is `index.ts`, `hukum`, or
// `hukum.exe` (see src/index.ts) - the same name the SEA binary uses.
// A `.cjs`/`.js` name silently fails that guard (no output, exit 0).
// Node runs the extensionless, `"use strict"` CJS bundle as CommonJS.
const BUNDLE_BASENAME = "hukum";
...
const PUBLISH_NAME = "@hukumai/cli";
...
function main() {
  ...
  // Version precedence: the release workflow injects HUKUM_CLI_VERSION
  ...
  const cliVersion =
    typeof process.env.HUKUM_CLI_VERSION === "string" &&
    process.env.HUKUM_CLI_VERSION.length > 0
      ? process.env.HUKUM_CLI_VERSION
  ...
  const args = [
    ...
    `--define:process.env.HUKUM_CLI_VERSION=${JSON.stringify(cliVersion)}`,
  ];
  ...
  //
  // Version shim: the CLI resolves `hukum --version` from
  // process.env.HUKUM_CLI_VERSION at RUNTIME - src reads it via
  ...
  code = code.replace(/^#![^\n]*\n/, "");
  const versionShim = `process.env.HUKUM_CLI_VERSION=${JSON.stringify(cliVersion)};`;
  const distributionShim = `process.env.HUKUM_CLI_DISTRIBUTION="npm";`;
  ...
  // "hukum" entry) so the published command stays in lockstep.
  const binCommand =
    Object.keys(workspaceManifest.bin || { hukum: "" })[0] || "hukum";
  ...
  const publishManifest = {
    ...
    description:
      ...
      "Hukum CLI - host supervisor, auth, and config surface",
    // No "type": the single extensionless `hukum` bundle is CommonJS (it
    ...
    repository: workspaceManifest.repository || {
      type: "git",
      url: "git+https://github.com/hukumai/hukum.git",
      directory: "clients/hukum-cli",
    },

 ...
 * backend base URLs have ONE dev-gated env override (`HUKUM_DEV_*_BASE_URL`,
 * validated loopback-only in `@hukum-clients/shared/platform/dev-backend-urls`)
...
const bakedConfig = {
  ...
  // real releases out of the box. The ~/.hukum/cli/host-trusted-pubkeys
  // disk overlay can ADD keys (e.g. mid-rotation) without a rebuild.
  hostTrustedPubkeys: [
    // Hukum host release signing key (minisign, key id 847ef539119a1961).
...
const DEFAULT_RELEASE_REPO = "hukum/hukum";

...
/**
 * `hukum worktree list` - host-wide listing of every Hukum-managed
 * worktree under `~/.hukum/worktrees/`. Calls `worktree.listAllForHost@1.4`;
...
export function parseWorktreeListLimit(value: string | null): number | null {
  ...
  throw cliError({
    ...
    message: "hukum worktree list: --limit must be a positive integer.",
    details: { limit: value },
    exitCode: 1,
  });
}
...
function formatWorktreeListTailHints(
  ...
  const outputLines =
    lines.length === 0 ? ["No Hukum-managed worktrees found."] : [...lines];
  if (nextCursor !== null) {
    outputLines.push(
      "",

...
/**
 * `hukum agent title-from-hook --provider <provider>` - invoked by a
 ...
 *
 * The command is intentionally lenient: missing `HUKUM_EPIC_ID` /
 * `HUKUM_AGENT_ID`, an empty prompt, an unparsable JSON payload, a
 ...
 * outside Hukum), so any benign condition must be a silent no-op.

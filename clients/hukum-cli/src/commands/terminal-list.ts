import { listTerminalsResponseSchemaV22 } from "@hukum/protocol/host/terminal/unary-schemas";
import type { CanonicalTerminalSessionInfoWithCurrentCwd } from "@hukum/protocol/host/terminal/unary-schemas";
import {
  callHostRpc,
  parseHostResponse,
  toAgentCliError,
...
/**
 * `hukum terminal list` - the interactive terminals in this Task you can
 ...
 *
 * `terminal-agent` sessions are filtered out: those back another Hukum
 * agent, whose conversation is `hukum agent transcript`'s job. Their PTY
...
export function formatTerminalListTable(
  ...
  return [
    ...
    ...rows.map(renderRow),
    "",
    "Read one with `hukum terminal output <terminal-id>`.",
  ].join("\n");
}

...
// `hukum cli upgrade` - self-upgrade the installed CLI binary.
//
...
const UPGRADE_HINT_FOR_SOURCE: Record<CliInstallSource, string> = {
  desktop: "Run 'hukum cli upgrade' (Desktop-owned).",
  manual: "Run 'hukum cli upgrade' (manual install).",
    ...
    return withCliLock(
      ...
      async () => {
        ...
        if (manifest === null) {
          ...
          throw cliError({
            ...
            message:
              ...
              "Run 'hukum cli re-anchor --binary-path <path> --installed-version <version>' " +
        ...
        const stagingRoot = installDirWritable
          ? installDir
          : await mkdtemp(join(tmpdir(), "hukum-cli-upgrade-"));
        ...
        const stagedBinaryPath = join(
          stagingRoot,
          `hukum-${targetVersion}-${platformKey}${binaryExtension()}`,
        );
        ...
        return {
          ...
          human:
            ...
            "Restart the host service ('hukum host restart') to finalise the swap.",
          exitCode: 0,
        };
      },
    );
...
async function directoryWritable(dirPath: string): Promise<boolean> {
  const probe = join(dirPath, `.hukum-upgrade-probe-${process.pid}`);
  try {

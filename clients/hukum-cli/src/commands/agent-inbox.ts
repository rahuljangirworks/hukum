import { formatAgentMessageSenderLabel } from "@hukum/protocol/agent/a2a-message-format";
import { agentInboxReadResponseSchemaV20 } from "@hukum/protocol/host/agent/inbox";
import {
  callHostRpc,
  parseHostResponse,
  toAgentCliError,
...
/**
 * `hukum agent inbox` - print the calling agent's recently-delivered
 * inbox messages in full (`agent.inbox.read`).
 *
 * The `hukum monitor` stream surfaces each inbound message through a
 ...
 *
 * `--agent-id` defaults to `$HUKUM_AGENT_ID`; the epic is always read from
 * `$HUKUM_EPIC_ID`, so an agent normally runs it with no flags.
 */
export function buildAgentInboxCommand(opts: {
  ...
  return async () => {
    ...
    const human = messages
      .map((message) => {
        ...
        const reply = message.reply.expectsReply
          ? ` — reply with: hukum agent send --to ${message.fromAgentId} --response-id ${message.reply.responseId} --message "<reply>"`
          : "";
    ...
    const nextPage =
      nextCursor === null
        ? ""
        : `\n\nMore inbox messages remain. Continue with: hukum agent inbox --after ${String(nextCursor.createdAt)}:${nextCursor.eventId}`;
    return {
      data: { messages, nextCursor },
...
function parseCursor(value: string | null): {
  ...
  if (
    ...
    throw cliError({
      ...
      message:
        "hukum: --after must use the createdAt:eventId cursor returned by the prior inbox page",
      details: null,
      exitCode: 1,
    });
  }

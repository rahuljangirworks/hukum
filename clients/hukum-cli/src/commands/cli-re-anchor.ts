...
// `hukum cli re-anchor --binary-path <path> --installed-version <version>` -
...
// `cli mark-source --source homebrew --binary-path /usr/local/bin/hukum`
// on a manually-installed binary would silently route every future
// upgrade through `brew upgrade hukum`, which won't find the
// install.

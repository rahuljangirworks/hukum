...
// Stream timing knobs, mirroring `hukum monitor`. A worktree delete is a
...
export interface WorktreeDeleteCommandOpts {
  ...
  // is true. Resolved from `HUKUM_AGENT_CLI_SURFACE` at registration.
  readonly readonlySurface: boolean;
}

/**
 * `hukum worktree delete --path <p>` - drives the host's deletion pipeline
  ...
  return async (ctx) => {
    if (opts.readonlySurface) {
      throw cliError({
        ...
        message:
          "hukum: worktree delete is not available in the readonly agent surface - remove worktrees from Settings ▸ Worktrees, or run this from a full-surface session.",
        details: null,
        exitCode: 1,
      });
    }
    ...
    if (worktreePath.length === 0) {
      throw cliError({
        ...
        message: "hukum: worktree delete requires --path <worktree path>.",
        details: null,
        exitCode: 1,
      });
    }
...
async function runWorktreeDelete(
  ...
  if (auth === null) {
    throw cliError({
      ...
      message: "hukum: not signed in - run `hukum login` to authenticate.",
      details: null,
      exitCode: 1,
    });
  }
...
function relayStatus(ctx: CommandContext, message: string): void {
  ...
  writeStderr(`[hukum worktree delete] ${message}\n`);
}
...
function deleteFailedCliError(reason: string): CliError {
  return cliError({
    ...
    message: `hukum: worktree delete failed - ${reason}`,
    details: null,
    exitCode: 1,
  });
}
...
function streamDroppedCliError(): CliError {
  return cliError({
    code: CLI_ERROR_CODES.UNEXPECTED,
    message:
      "hukum: worktree delete stream dropped before the host reported a result - the worktree may or may not have been removed. Run `hukum worktree list` to check, then retry if needed.",
    details: null,
    exitCode: 1,
  });
}
...
function fatalCloseToCliError(details: FatalErrorDetails): CliError {
  if (details.code === "UNAUTHORIZED") {
    return cliError({
      ...
      message:
        "hukum: host rejected the credential for worktree delete - run `hukum login` and retry.",
      details: null,
      exitCode: 1,
    });
  }
  if (
    ...
    return cliError({
      ...
      message: `hukum: ${details.reason} - update the host or CLI so their worktree delete versions match.`,
      details: null,
      exitCode: 1,
    });
  }
  return cliError({
    code: CLI_ERROR_CODES.UNEXPECTED,
    message: `hukum: worktree delete stream closed - ${details.reason}`,
    details: null,
    exitCode: 1,
  });
}

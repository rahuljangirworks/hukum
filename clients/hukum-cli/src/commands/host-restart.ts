...
// `hukum host restart` - kicks the OS service so the supervisor
...
// `hukum cli upgrade` stages the new binary and records
...
function humanForRestart(
  ...
  switch (outcome.status) {
    ...
    case "staged-binary-missing":
      return `${reconcilePrefix}${base}; cli upgrade staged binary ${outcome.stagedBinaryPath} missing - re-run 'hukum cli upgrade'`;
    case "no-pending":
    case "no-manifest":

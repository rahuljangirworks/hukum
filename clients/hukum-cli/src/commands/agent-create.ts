...
/**
 * `hukum agent create` - mint a child agent (`agent.create`). The new
...
function requireAbsolutePath(rawPath: string, label: string): string {
  ...
  if (trimmed.length === 0 || !path.isAbsolute(trimmed)) {
    throw cliError({
      ...
      message: `agent create: ${label} must be an absolute path. Use --cwd <worktree-path> for a path returned by hukum worktree create, or --workspace-entry <source-path>=<run-path> for an exact binding.`,

...
// Runner-aware `hukum logout`. JSON mode emits exactly one terminal
...
export const logoutCommand: CommandFn = async (ctx): Promise<CommandResult> => {
  ...
  if (signOut.outcome !== "deleted") {
    ...
    // (another hukum process holds the credentials lock): the credential was
    // NOT cleared, so we must not claim signed-out.
    throw cliError({
      code: CLI_ERROR_CODES.UNEXPECTED,
      message:
        "Logout failed to clear the stored credentials - another hukum process may be busy; please try again.",
      details: null,
      exitCode: 1,
    });
  }

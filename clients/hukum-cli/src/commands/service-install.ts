...
// `hukum host service install [--no-linger] [--takeover]` - register the
...
// the Hukum Desktop app to the CLI: the Desktop-managed host is stopped
    ...
    return withCliLock(
      ...
      async () => {
        ...
        if (args.takeover) {
          ctx.progress({
            stage: "register",
            message: `taking over host management from Hukum Desktop`,
            percent: null,
            bytes: null,
            totalBytes: null,
          });
        ...
        return {
          ...
          human:
            takeover !== null && takeover.kind === "took-over"
              ? `service '${label.id}' registered (environment=${label.environment}); host management taken over from Hukum Desktop (agent '${takeover.agentLabelId}' deregistered, host ${takeover.cooperativeStop === "stopped" ? "stopped cooperatively" : takeover.cooperativeStop === "no-host" ? "was not running" : "was unreachable and booted out"})`

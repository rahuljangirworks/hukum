import { formatAgentMessage } from "@hukum/protocol/agent/a2a-message-format";
import {
...
import type { RoleAwarenessEvent } from "@hukum/protocol/host/agent/roles";
...
/**
 * `hukum monitor` — long-running background command spawned inside a Claude
 * Code TUI session by the Hukum plugin. It subscribes to the host's
...
export async function runMonitor(args: MonitorArgs): Promise<void> {
  ...
  const agentId = args.agentId ?? process.env.HUKUM_AGENT_ID ?? null;
  const epicId = args.epicId ?? process.env.HUKUM_EPIC_ID ?? null;
  ...
  if (agentId === null || agentId.length === 0) {
    ...
    throw new Error(
      "hukum monitor: agent id required — pass --agent-id or set HUKUM_AGENT_ID.",
    );
  }
  if (epicId === null || epicId.length === 0) {
    ...
    throw new Error("hukum monitor: epic id required — set HUKUM_EPIC_ID.");
  }
  ...
  if (auth === null) {
    ...
    throw new Error(
      "hukum monitor: not signed in — run `hukum login` to authenticate.",
    );
  }
 ...
 * so `hukum monitor` exits non-zero.
 *
  ...
  return new Promise<never>((_resolve, reject) => {
    ...
    const onStatusChange = async (
      ...
      if (reason.details.code !== "UNAUTHORIZED") {
        ...
        fail(
          new Error(
            `hukum monitor: host closed the stream: ${reason.details.reason}`,
          ),
        );
        return;
      }
      ...
      if (outcome === "rotated") {
        ...
        if (authRefreshCount > MAX_CONSECUTIVE_AUTH_REFRESHES) {
          ...
          fail(
            new Error(
              `hukum monitor: session rejected after ${authRefreshCount} refreshes — the agent/epic may be invalid or inaccessible (check --agent-id and HUKUM_EPIC_ID).`,
            ),
          );
          return;
        }
      ...
      fail(new Error("hukum monitor: session expired — re-authenticate."));
    };
...
function printInboxNotice(notice: AgentInboxNotice): void {
  ...
  const lines = [
    "",
    `[hukum inbox] inactivity notice — ${inactivityHeadline(notice, receiverLabel)}${harnessSuffix} (responseId ${notice.responseId})`,
    `[hukum inbox] check what it is doing: hukum agent transcript --agent-id ${notice.receiverAgentId}`,
    `[hukum inbox] the request is still open; a follow-up on the same thread can be sent with: hukum agent send --to ${notice.receiverAgentId} --response-id ${notice.responseId} --message "<follow-up>"`,
    `[hukum inbox] based on your judgment decide how to proceed — read transcript, follow up, launch a new agent, etc.`,
    "",
  ];
...
function printRoleAwareness(event: RoleAwarenessEvent): void {
  ...
  writeStdout(
    `[hukum roles] agent ${event.claim.agentId} ${verb} "${event.claim.role}" (scope: ${event.claim.scope})\n`,
  );
}
...
function printReceiverCancelledNotice(
  ...
  const headlineLines = plural
    ? [
        `[hukum inbox] inactivity notice — ${dropped.length} messages you sent could not be delivered; the user stopped the agents you were waiting on:`,
        ...dropped.map(
          (thread) =>
            `[hukum inbox]   · agent ${thread.receiverAgentId} (responseId ${thread.responseId})`,
        ),
      ]
    : [
        `[hukum inbox] inactivity notice — ${inactivityHeadline(notice, receiverLabel)}${harnessSuffix} (responseId ${notice.responseId})`,
  ...
  const lines = [
    "",
    ...headlineLines,
    `[hukum inbox] this is informational only — do NOT re-send ${plural ? "them" : "the message"} or launch ${plural ? "new agents" : "a new agent"} to take their place`,
    `[hukum inbox] if you are working on the user's behalf, wait for their next instruction; if you are working on behalf of another agent, let that agent know`,
    "",
  ];
  writeStdout(`${lines.join("\n")}\n`);
}
...
function diag(message: string): void {
  writeStderr(`[hukum monitor] ${message}\n`);
}

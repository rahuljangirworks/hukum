...
export async function resolveWorktreeBranchSelection(
  ...
  if (opts.newBranch !== null && opts.existingBranch !== null) {
    throw cliError({
      ...
      message:
        "hukum: --branch (create a new branch) and --existing (check out an existing branch) cannot be combined - pass exactly one.",
      details: null,
      exitCode: 1,
    });
  }
  ...
  if (opts.existingBranch !== null) {
    ...
    if (opts.sourceBranch !== null) {
      throw cliError({
        code: CLI_ERROR_CODES.INVALID_ARGUMENT,
        message:
          "hukum: --source-branch only applies to --branch (creating a new branch); it cannot be combined with --existing.",
        details: null,
        exitCode: 1,
      });
    }
    ...
    if (name.length === 0) {
      throw cliError({
        code: CLI_ERROR_CODES.INVALID_ARGUMENT,
        message: "hukum: --existing requires a non-empty branch name.",
        details: null,
        exitCode: 1,
      });
    }
  ...
  if (name.length === 0) {
    throw cliError({
      code: CLI_ERROR_CODES.INVALID_ARGUMENT,
      message:
        "hukum: pass --branch <name> to create a new branch or --existing <name> to check out an existing one.",
      details: null,
      exitCode: 1,
    });
  }
...
async function resolveCurrentBranch(workspacePath: string): Promise<string> {
  ...
  if (current === undefined) {
    throw cliError({
      code: CLI_ERROR_CODES.INVALID_ARGUMENT,
      message:
        "hukum: could not resolve the workspace's current branch (detached HEAD or not a Git worktree) - pass --source-branch <branch> explicitly.",
      details: null,
      exitCode: 1,
    });
  }

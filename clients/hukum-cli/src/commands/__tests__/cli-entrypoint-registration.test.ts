...
// Native-packaging follow-up bug: previously `hukum-cli/src/index.ts`
...
// implemented but never registered, so `hukum host doctor --json`
...
describe("hukum CLI entrypoint registration", () => {
  ...
  it("threads --json / --no-progress through every runner-aware command", () => {
    // Long-running operations are invoked via streamHukumCliJson and
    // depend on --json to switch the shared runner into NDJSON mode.
    // Short-lived ones invoked via runHukumCliJson also need --json
  ...
  it("limits readonly agent CLI help to inspection commands", () => {
    const originalSurface = process.env.HUKUM_AGENT_CLI_SURFACE;
    process.env.HUKUM_AGENT_CLI_SURFACE = "readonly";
    try {
      const program = buildProgram();
      const agent = expectCommand(program, ["agent"]);
      const help = agent.helpInformation();
      ...
      if (originalSurface === undefined) {
        delete process.env.HUKUM_AGENT_CLI_SURFACE;
      } else {
        process.env.HUKUM_AGENT_CLI_SURFACE = originalSurface;
      }
    }
  });
  ...
  // Service manifests render argv as `hukum host start` - the slot is

...
describe("cliFinalizeUpgradeCommand / runFinalizeUpgradeSwap", () => {
  beforeEach(() => {
    workHome = mkdtempSync(
      join(tmpdir(), "hukum-cli-finalize-upgrade-test-"),
    );
  ...
  function markerPath(): string {
    return join(workHome, ".hukum", "cli", "post-finalize.json");
  }
  ...
  it("on a finalised swap, starts the service and writes a 'swapped' marker", async () => {
    mocks.finalizeResult = {
      ...
      version: "1.5.0",
      binaryPath: "/opt/hukum/cli/hukum",
    };
  ...
  // job for hukum-cli (`.github/workflows/test.yml` runs only
  ...
  it("on still-locked, writes a 'swap-failed' marker and never starts the service", async () => {
    mocks.finalizeResult = {
      status: "still-locked",
      stagedBinaryPath: "/opt/hukum/cli/hukum-1.5.0",
      livePath: "/opt/hukum/cli/hukum",
    ...
    async (status) => {
      mocks.finalizeResult =
        status === "staged-binary-missing"
          ? { status, stagedBinaryPath: "/opt/hukum/cli/hukum-1.5.0" }
          : { status };

/**
 * `hukum agent role` command functions and Commander registration.
 *
...
} from "@hukum/protocol/agent/agent-roles-format";
import type {
...
describe("agent role claim command function", () => {
  ...
  it("defaults ids from $HUKUM_EPIC_ID / $HUKUM_AGENT_ID; explicit flags win", async () => {
    callHostRpcMock.mockResolvedValue(CLAIM_RESPONSE);
    process.env.HUKUM_EPIC_ID = "epic-env";
    process.env.HUKUM_AGENT_ID = "agent-env";

...
beforeEach(() => {
  ...
  process.env.HUKUM_AGENT_ID = "agent-1";
});
...
describe("buildAgentTitleFromHookCommand", () => {
  ...
  it("exits cleanly without an RPC call when HUKUM_EPIC_ID is missing", async () => {
    delete process.env.HUKUM_EPIC_ID;
    stubStdin({
      isTTY: false,
  ...
  it("exits cleanly without an RPC call when HUKUM_AGENT_ID is missing", async () => {
    delete process.env.HUKUM_AGENT_ID;
    stubStdin({
      isTTY: false,
      chunks: [JSON.stringify({ prompt: "hi" })],
  ...
  it("noops with exit 0 when the host is not running", async () => {
    ...
    rpcMock.mockRejectedValueOnce(
      cliError({
        ...
        message: "hukum: host not running",
        details: null,
        exitCode: 1,
      }),
    );

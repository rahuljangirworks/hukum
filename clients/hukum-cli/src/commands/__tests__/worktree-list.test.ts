...
import type { WorktreeHostEntryV14 } from "@hukum/protocol/host";
...
describe("formatWorktreeListTable", () => {
  it("returns an explicit empty-state line when there are no worktrees", () => {
    expect(formatWorktreeListTable([], true, null)).toBe(
      "No Hukum-managed worktrees found.",
    );
  });
  ...
  it("renders a header row and one row per worktree", () => {
    ...
    expect(lines[1]).toContain(
      "/Users/dev/.hukum/worktrees/acme__web/feature-x",
    );
  });
  ...
  it("appends a resume hint when a single-page response has a next cursor", () => {
    ...
    expect(table).toContain(
      "--cursor /Users/dev/.hukum/worktrees/acme__web/feature-x",
    );
  });
});
...
describe("buildWorktreeListCommand", () => {
  ...
  it("auto-pages by default and aggregates rows into one result", async () => {
    const first = entry({
      worktreePath: "/Users/dev/.hukum/worktrees/acme__api/feature-a",
      repoLabel: "acme/api",
      branch: "feature/a",
    });
    const second = entry({
      worktreePath: "/Users/dev/.hukum/worktrees/acme__web/feature-b",
      repoLabel: "acme/web",
      branch: "feature/b",
    });
    rpcMock
  ...
  it("passes explicit cursor and limit through as a single-page request", async () => {
    ...
    rpcMock.mockResolvedValue({
      worktrees,
      nextCursor: "/Users/dev/.hukum/worktrees/acme__web/feature-y",
    });
  ...
  it("auto-pages from an explicit cursor when no limit is provided", async () => {
    const startCursor = "/Users/dev/.hukum/worktrees/acme__api/feature-a";
    const first = entry({
      worktreePath: "/Users/dev/.hukum/worktrees/acme__web/feature-b",
      repoLabel: "acme/web",
      branch: "feature/b",
    ...
    const second = entry({
      worktreePath: "/Users/dev/.hukum/worktrees/acme__web/feature-c",
      repoLabel: "acme/web",
      branch: "feature/c",
    });
    rpcMock

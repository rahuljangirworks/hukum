...
// Stub the CLI logger so this state-machine test never appends to ~/.hukum.
vi.mock("../../logger", () => ({
...
describe("role awareness frames (negotiated @1.1)", () => {
  it("prints one compact [hukum roles] line per event, for both claim and relinquish", async () => {
    const stdoutSpy = vi
    ...
    expect(lines).toContain(
      '[hukum roles] agent peer-1 claimed role "Planner" (scope: auth migration)\n',
    );
    expect(lines).toContain(
      '[hukum roles] agent peer-1 relinquished role "Planner" (scope: auth migration)\n',
    );
  ...
  it("drops a malformed role-awareness frame without printing", async () => {
    ...
    const lines = stdoutSpy.mock.calls.map((call) => String(call[0]));
    expect(lines.filter((line) => line.includes("[hukum roles]"))).toEqual(
      [],
    );

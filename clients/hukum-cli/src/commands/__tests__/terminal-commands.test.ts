...
beforeEach(() => {
  vi.clearAllMocks();
  process.env.HUKUM_EPIC_ID = "epic-1";
});
...
describe("formatTerminalListTable", () => {
  ...
  it("renders a header row, one row per terminal, and how to read one", () => {
    ...
    expect(table).toContain("hukum terminal output <terminal-id>");
  });
...
describe("buildTerminalOutputCommand", () => {
  it("prints where the output landed instead of dumping it", async () => {
    rpcMock.mockResolvedValue({ path: "/tmp/hukum-terminal-output/t1.txt" });
    ...
    expect(result.data).toEqual({
      path: "/tmp/hukum-terminal-output/t1.txt",
    });
    expect(result.human).toBe(
      "terminal output written to /tmp/hukum-terminal-output/t1.txt",
    );
    expect(result.exitCode).toBe(0);
  });
  ...
  it("addresses the Task the caller names rather than the ambient one", async () => {
    ...
    // Same allowance `hukum agent transcript` has: the read is Task-scoped,

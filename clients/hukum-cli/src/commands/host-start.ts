...
// `hukum host start` is the long-running supervisor invoked by the OS
...
//   1. Read ~/.hukum/host[/dev]/install/install.json.
...
//      stages a tiny wrapper script under `~/.hukum/host/dev/` that
...
export async function resolveHostStartTarget(
  ...
  if (record === null) {
    throw cliError({
      ...
      message: `no host installed on environment '${opts.environment}'. Run 'hukum host install latest' to install one.`,
  ...
  if (!(await deps.pathExists(record.executablePath))) {
    throw cliError({
      code: CLI_ERROR_CODES.HOST_NOT_INSTALLED,
      message: `host executable missing on environment '${opts.environment}' at ${record.executablePath}. Re-run 'hukum host install latest'.`,
      details: {
...
export async function runHostStart(
  ...
  // `hukum host start` and waiting on the result?
  //
  ...
  // publishing - a raw `hukum host start` against a running host, one
  ...
  // (`ai.hukum.host`) and Desktop's SMAppService label
  // (`ai.hukum.host.agent`), since both set `RunAtLoad`. pid.json does not
  ...
  for (;;) {
    ...
    // describes as only partly covered. `hukum host ensure` can legitimately
    ...
    try {
      ...
      // (`serviceStarted`). A person or Desktop running `hukum host start`
      ...
      if (err instanceof CliError) {
        ...
        deps.onError(`hukum host start: ${err.code}: ${err.message}`);
    ...
    try {
      ...
      // computes its own CLI bin dir (`~/.hukum/cli[/<slot>]/bin`, where the
      // bundled `hukum` is symlinked) and puts it on PATH, so no `hukum` path
      ...
      deps.onError(
        err instanceof CliError
          ? `hukum host start: ${err.code}: ${err.message}`
          : `hukum host start: ${failure.message}`,
      );
      return exitSupervisor(err instanceof CliError ? err.exitCode : 1);
    }
    ...
    // "C:\Users\Hukum Dev\..."), so cmd splits it and fails with
    // "'C:\Users\Hukum' is not recognized". Invoke cmd.exe ourselves with a
    ...
    try {
      ...
      deps.onError(
        `hukum host start: ${CLI_ERROR_CODES.HOST_SPAWN_FAILED}: ${message}`,
      );
      await deps.closeLogFd(logFd);
 ...
 * `hukum host stop` or a `launchctl bootout` arriving DURING that backoff was
...
// cmd.exe unquoted and it fails with "'C:\Users\Hukum' is not recognized".

...
// Runner-aware `hukum whoami`. JSON mode emits exactly one terminal
...
export const whoamiCommand: CommandFn = async (ctx): Promise<CommandResult> => {
  ...
  if (result.kind === "no-credentials") {
    return {
      ...
      human: ctx.runtime.json
        ? null
        : "Not logged in. Run `hukum login` to authenticate.",
      exitCode: 1,
    };
  }
  if (result.kind === "rejected") {
    return {
      ...
      human: ctx.runtime.json
        ? null
        : "Stored credentials were rejected by the authn service. Run `hukum login` to re-authenticate.",
      exitCode: 1,
    };
  }

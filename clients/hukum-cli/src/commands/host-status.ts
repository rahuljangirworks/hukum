...
// Runner-aware `hukum host status`. Wires Core Flow 7 auto-bootstrap
...
//
// `--no-bootstrap`, `CI=1`, `HUKUM_NONINTERACTIVE=1` cause bootstrap
...
// data: ... }` line so Desktop can parse it through `runHukumCliJson`.
...
function renderHumanStatus(
  ...
  if (output.running && output.pidMetadata !== null) {
    ...
    lines.push(`${c.green("●")} ${c.bold("Hukum host is running")}`);
    lines.push("");
    lines.push(
      ...kvBlock(c, [
        ["PID", String(m.pid)],
    ...
    lines.push(`${c.gray("○")} ${c.bold("Hukum host is not running")}`);
    lines.push("");
...
function renderBootstrapLine(
  ...
  switch (decision.status) {
    ...
    case "skipped":
      ...
      return c.dim(
        "bootstrap: skipped (non-interactive - CI=1 or HUKUM_NONINTERACTIVE=1)",
      );
    case "failed":

...
// actual `~/.hukum` log file - stub it so the suite stays hermetic and the
...
const TEST_LAUNCH_AGENTS_DIR = mkdtempSync(
  join(tmpdir(), "hukum-macos-service-test-"),
);
...
describe("macOS service lifecycle", () => {
  ...
  it("gives the host an 8,192 soft file-descriptor limit", () => {
    const plist = buildLaunchAgentPlist({
      label,
      cli: { command: "/usr/local/bin/hukum", args: [] },
    });
  ...
  // the Hukum app in System Settings → Login Items.
  it("associates the LaunchAgent with the Hukum desktop app so Login Items does not show it as 'sh'", () => {
    const plist = buildLaunchAgentPlist({
      label,
      cli: { command: "/usr/local/bin/hukum", args: [] },
    });
  ...
  it("starts both an N-1 CLI without --service-label and a current CLI with it, preserving leading invocation args", async () => {
    ...
    const script = buildCompatibleHostStartScript("ai.hukum.host.compat");
    try {
...
`,
  ...
  // `~/.hukum/service/<label>/hukum-host-start <cli> <args...>` (so BTM
  ...
  it("launcher file: starts both an N-1 CLI without --service-label and a current CLI with it, preserving leading invocation args", async () => {
    const work = mkdtempSync(join(tmpdir(), "hukum-host-start-launcher-"));
    const launcher = join(work, "hukum-host-start");
    const oldCli = join(work, "old-cli.sh");
    const newCli = join(work, "new-cli.sh");
    const oldArgs = join(work, "old-args.txt");
    const newArgs = join(work, "new-args.txt");
    try {
      await writeFile(
        launcher,
        buildHostStartLauncherScript("ai.hukum.host.compat"),
        "utf8",
      );
...
`,
  ...
  it("puts the per-label launcher file first in ProgramArguments so BTM names the item hukum-host-start", () => {
    const plist = buildLaunchAgentPlist({
      label,
      cli: { command: "/usr/local/bin/hukum", args: ["--entry=e.js"] },
    });

    const launcherPath = serviceLauncherScriptPath(label);
    expect(launcherPath.endsWith(`/${label.id}/hukum-host-start`)).toBe(true);
    expect(plist).toContain(`<key>ProgramArguments</key>
  <array>
    <string>${launcherPath}</string>
    <string>/usr/local/bin/hukum</string>
  ...
  it("writes the plist and registers regardless of HUKUM_HOST_SKIP_SERVICE_REGISTER (env hack removed)", async () => {
    const previous = process.env.HUKUM_HOST_SKIP_SERVICE_REGISTER;
    process.env.HUKUM_HOST_SKIP_SERVICE_REGISTER = "1";
    try {
      const calls: RecordedCall[] = [];
      const runner: ProcessRunner = async (command, args) => {
        calls.push({ command, args });
      ...
      if (previous === undefined) {
        delete process.env.HUKUM_HOST_SKIP_SERVICE_REGISTER;
      } else {
        process.env.HUKUM_HOST_SKIP_SERVICE_REGISTER = previous;
      }
    }
  });
  ...
  it("detects SMAppService in-bundle LaunchAgent paths", () => {
    ...
    expect(
      isSmAppServiceLaunchAgentPath(
        "/Users/me/Applications/Hukum Staging.app/Contents/Library/LaunchAgents/ai.hukum.host.staging.plist",
      ),
    ).toBe(true);
    expect(
      isSmAppServiceLaunchAgentPath(
        "/Users/me/Library/LaunchAgents/ai.hukum.host.plist",
      ),
    ).toBe(false);
    expect(
      classifyLaunchdPrintOutput(
        `gui/501/ai.hukum.host = {\n\tpath = /Applications/Hukum.app/Contents/Library/LaunchAgents/ai.hukum.host.plist\n\tstate = running\n}\n`,
      ),
    ).toEqual({
      kind: "smappservice",
      path: "/Applications/Hukum.app/Contents/Library/LaunchAgents/ai.hukum.host.plist",
  ...
  it("classifies an SMAppService job that reports no bundle path", () => {
    const printOutput = [
      "gui/501/ai.hukum.host.staging.agent = {",
      "\tactive count = 1",
      ...
      "\tstate = running",
      "",
      "\tprogram identifier = Contents/Library/LaunchAgents/Hukum Staging Host.app/Contents/MacOS/hukum (mode: 2)",
      "\tparent bundle identifier = ai.hukum.desktop.staging",
      "\targuments = {",
      "\t\tContents/Library/LaunchAgents/Hukum Staging Host.app/Contents/MacOS/hukum",
      "\t\thost",
      "\t\tstart",
      "\t}",
      "",
      "\tenvironment = {",
      "\t\tOSLogRateLimit => 64",
      "\t\tXPC_SERVICE_NAME => ai.hukum.host.staging.agent",
      "\t}",
      "}",
      "",
    ].join("\n");
  ...
  it("classifies a raw CLI-bootstrapped LaunchAgent as cli-or-other", () => {
    const printOutput = [
      "gui/501/ai.hukum.host = {",
      "\tactive count = 1",
      "\tpath = /Users/me/Library/LaunchAgents/ai.hukum.host.plist",
      "\ttype = LaunchAgent",
      "\tstate = running",
      "",
      "\tprogram = /Users/me/.hukum/cli/bin/hukum",
      "\targuments = {",
      "\t\t/Users/me/.hukum/cli/bin/hukum",
      "\t\thost",
      "\t\tstart",
      "\t}",
      "}",
  ...
  it("reports externally-managed when launchd loads the label from an SMAppService path even if a stale raw plist exists", async () => {
    ...
    // installService's SMAppService refusal on every `hukum login`).
    createdPlistPath = join(tempPlistDir, `${label.id}.plist`);
  ...
  it("refuses install when the label is already loaded from an SMAppService path", async () => {
    ...
    expect(rejection).toMatchObject({
      message: expect.stringContaining("hukum host service uninstall"),
    });
  ...
  it("refuses install when Desktop's post-label-split AGENT label is SMAppService-loaded - the CLI label itself reads clean", async () => {
    ...
    const calls: RecordedCall[] = [];
    const smAgentPath =
      "/Applications/Hukum.app/Contents/Library/LaunchAgents/ai.hukum.host.agent.plist";
    const runner: ProcessRunner = async (command, args) => {
      calls.push({ command, args });
      if (args[0] === "print") {
        if (args[1]?.endsWith(".agent") === true) {
  ...
  it("reports externally-managed when only the post-label-split AGENT label is SMAppService-loaded", async () => {
    ...
    // installService's agent-label refusal on every `hukum login`.
    const smAgentPath =
      "/Applications/Hukum.app/Contents/Library/LaunchAgents/ai.hukum.host.agent.plist";
    const runner: ProcessRunner = async (command, args, options) => {
      if (args[0] === "print" && options.tolerateNonZeroExit) {
  ...
  describe("Desktop-managed stop/start/restart (cooperative, never a refusal)", () => {
    ...
    // Hukum app") - which cornered exactly the users whose Desktop app
    ...
    /*
     * The restart HALVES, which is what `hukum host restart` actually calls.
     *
  ...
  describe("takeoverDesktopRegistration", () => {
      ...
      const runner: ProcessRunner = async (command, args) => {
        ...
        if (args[0] === "print") {
          ...
          return input.cliLabelOwned
            ? {
                stdout: `path = /Applications/Hukum.app/Contents/Library/LaunchAgents/${label.id}.plist\n`,
                stderr: "",
                exitCode: 0,
              }
            : {
    ...
    it("cooperatively stops, boots out the agent, and verifies the bootout took effect", async () => {
      ...
      expect(MOCKS.cliLoggerInfo).toHaveBeenCalledWith(
        expect.stringContaining("booted out Hukum Desktop's SMAppService"),
  ...
  describe("readRegisteredCliInvocation (host update's no-repoint contract)", () => {
    ...
    it("refuses a launcher-form manifest whose path is not this label's own serviceLauncherScriptPath", async () => {
      ...
      // `hukum-host-start` basename alone would treat this as a genuine
      ...
      await writeFile(
        createdPlistPath,
        `<plist><dict><key>ProgramArguments</key><array><string>/tmp/attacker-controlled/hukum-host-start</string><string>${process.execPath}</string></array></dict></plist>`,
        "utf8",
      );

...
function stagedTaskInstallDeps(): WindowsTaskInstallDeps {
  return {
    stageTaskDefinition: async () => ({
      tmpDir: "/tmp/hukum-task-test",
      xmlPath: "/tmp/hukum-task-test/task.xml",
    }),
...
describe("Windows service stale host cleanup", () => {
  ...
  it("builds a slot-scoped process scan that excludes the current CLI process", () => {
    const script = buildWindowsSlotProcessScanScript({
      hostHome: "C:\\Users\\Hukum Dev\\.hukum\\host\\staging",
      currentPid: 1234,
    });
    expect(script).toContain("$excluded = @(1234, $PID)");
    expect(script).toContain(".hukum\\host\\staging\\install\\");
    expect(script).toContain("Replace('/', '\\')");
  });

  it("does not use broad production roots as process-match prefixes", () => {
    const script = buildWindowsSlotProcessScanScript({
      hostHome: "C:\\Users\\Hukum Dev\\.hukum\\host",
      currentPid: 1234,
    });
    expect(script).toContain(
      "c:\\users\\hukum dev\\.hukum\\host\\install\\",
    );
    expect(script).not.toContain("'c:\\users\\hukum dev\\.hukum\\host'");
  });
  ...
  it("kills slot-scanned processes when pid metadata is missing", async () => {
    ...
    expect(calls[0]).toMatchObject({
      command: "schtasks",
      args: ["/End", "/TN", "\\Hukum\\Host-Staging"],
    });
...
describe("Scheduled Task XML identity", () => {
  it("names Hukum as the task Author", () => {
    ...
    try {
      ...
      expect(xml).toContain("<Author>Hukum</Author>");
    } finally {
...
describe("parseSchtasksLastRunResult", () => {
  ...
  it("reads the positional result from localized headerless CSV", () => {
    const localizedCsv =
      '"ORDINATEUR","\\\\Hukum\\\\Host","N/A","Ready","Interactive only","01/01/2026 00:00:00","0x41301","user","wscript.exe //B host-start-hidden.vbs"';
    ...
    expect(
      calls.some(
        (call) =>
          ...
          call.args[0] === "/Run" &&
          call.args.includes("\\Hukum\\Host-Staging"),
      ),
    ).toBe(true);
    expect(
      calls.some(

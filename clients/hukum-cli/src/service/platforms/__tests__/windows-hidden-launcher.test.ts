...
describe("Windows hidden host launcher — decoded as Windows decodes it", () => {
  const cli = {
    command: "C:\\Users\\Hukum Dev\\.hukum\\cli\\bin\\hukum.exe",
    args: [],
  };
  const serviceLabel = label("ai.hukum.host.prod");
  ...
  it("hands the capability probe an argv Windows can actually resolve", () => {
    ...
    // ["cmd.exe","/d","/s","/c", '\\"C:\\Users\\...\\hukum.exe\\" ...'],
    // whose leading literal backslash cmd.exe could not resolve.
    expect(probe).toEqual([
      "C:\\Users\\Hukum Dev\\.hukum\\cli\\bin\\hukum.exe",
      "host",
      "capabilities",
      "--has",
      "service-label",
  ...
  it("decodes the labelled start line to the exact argv, including leading invocation args", () => {
    const script = buildWindowsHiddenHostLauncher(
      {
        command: "C:\\Program Files\\Hukum\\hukum.exe",
    ...
    expect(
      ...
      "C:\\Program Files\\Hukum\\hukum.exe",
      "--entry=cli-entry.js",
      "host",
      "start",
      "--service-label",

 ...
 * left behind. `attestHukumRegistration` therefore stopped recognising the
 ...
 * `isEvictableHukumIdentity` refuses to evict on `indeterminate` — so the
...
describe("host-start script / identity attestation lockstep", () => {
  const labelId = "ai.hukum.host.dev";
  ...
  it("attests a registration built by the real emitter, on label shape alone", () => {
    ...
    expect(isEvictableHukumIdentity(attestation)).toBe(true);
  });
  ...
  it("attests a launcher-file registration built by the real path helper, on label shape alone", () => {
    ...
    // (`~/.hukum/service/<label>/hukum-host-start`), so there is no
    ...
    const attestation = attestHukumRegistration({
      labelId,
      knownLabels: null,
      programArguments: [launcherPath, "/usr/local/bin/hukum"],
      contentTag: null,
      sourceText: null,
    });
  ...
  it("does not attest a launcher whose label sits ABOVE its immediate parent directory", () => {
    ...
    const attestation = attestHukumRegistration({
      ...
      knownLabels: null,
      programArguments: [
        `/Users/u/.hukum/service/${labelId}/nested/${HOST_START_LAUNCHER_BASENAME}`,
        "/usr/local/bin/hukum",
      ],
      contentTag: null,
      sourceText: null,
    ...
    expect(isEvictableHukumIdentity(attestation)).toBe(false);
  });
  ...
  it("does not attest a launcher-shaped path with no label context", () => {
    ...
    // `.../hukum-host-start` from attesting as ours merely because no
    ...
    // (`not-hukum`), not merely `indeterminate`.
    const attestation = attestHukumRegistration({
      labelId: null,
      knownLabels: null,
      programArguments: [
    ...
    expect(attestation.kind).toBe("not-hukum");
    expect(isEvictableHukumIdentity(attestation)).toBe(false);
  });
  ...
  it("does not attest a launcher registered for a DIFFERENT label as this label's invocation", () => {
    ...
    const attestation = attestHukumRegistration({
      ...
      knownLabels: null,
      programArguments: [
        `/Users/u/.hukum/service/ai.hukum.host.staging/${HOST_START_LAUNCHER_BASENAME}`,
        "/usr/local/bin/hukum",
      ],
      contentTag: null,
      sourceText: null,
  ...
  it("still refuses to evict a foreign script on the same label shape", () => {
    ...
    //
    // The verdict is `indeterminate`, not `not-hukum`, and that is the
    // designed behaviour rather than a weaker result — a Hukum-shaped label

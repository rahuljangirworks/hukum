 ...
 * `attestHukumRegistration` must agree on this string byte-for-byte, and a
 ...
 * above: `attestHukumRegistration` recognises launcher-form plists by
 ...
 * makes BTM record `Name: hukum-host-start` - probe-verified on a live
...
export function buildHostStartLauncherScript(serviceLabel: string): string {
  return `#!/bin/sh
# Hukum host launcher. Written by 'hukum host service install'; the

...
const defaultTaskInstallDeps: WindowsTaskInstallDeps = {
  stageTaskDefinition: async (options) => {
    const tmpDir = await mkdtemp(join(tmpdir(), "hukum-task-"));
...
async function uninstallService(
  ...
  // `schtasks /Delete` removes only the task; the `\Hukum` FOLDER it was
  ...
  await run(
    ...
    [
      ...
      "-NonInteractive",
      "-Command",
      "$s=New-Object -ComObject Schedule.Service;$s.Connect();$f=$s.GetFolder('\\Hukum');if((@($f.GetTasks(1)).Count -eq 0) -and (@($f.GetFolders(0)).Count -eq 0)){$s.GetFolder('\\').DeleteFolder('Hukum',0)}",
    ],
    {
      env: undefined,
      cwd: undefined,
...
function resolveTaskUserId(): string {
  ...
  throw cliError({
    ...
    message:
      ...
      "Run `hukum host service install` from an interactive logon session.",

...
async function takeoverDesktopRegistration(
  ...
  if (cliOwnership.kind === "smappservice") {
    throw cliError({
      ...
      message: `service install --takeover: label '${label.id}' is Desktop's own SMAppService registration (pre-label-split machine, loaded from ${cliOwnership.path}); takeover cannot bootout this label without corrupting the login-item state the app manages. Run 'hukum host service uninstall', then re-run 'hukum host service install'.`,
    ...
    throw cliError({
      ...
      message:
        postBootout === "still-loaded"
          ? `service install --takeover: launchctl bootout of '${desktopAgent.agentLabelId}' did not take effect (the agent is still loaded). Use the Hukum app to remove the host, or run 'hukum host service uninstall'.`
          : `service install --takeover: could not confirm that '${desktopAgent.agentLabelId}' was booted out (launchctl did not answer), so the takeover was stopped rather than risk running two hosts. Re-run the command, or use the Hukum app to remove the host, or run 'hukum host service uninstall'.`,
      details: {
        label: label.id,
        agentLabel: desktopAgent.agentLabelId,
  ...
  logger.info(
    "Takeover: booted out Hukum Desktop's SMAppService agent; the CLI now owns host registration. Launching the Desktop app again may re-register its agent - uninstall or update the app to make the takeover permanent.",
    {
      label: label.id,
      agentLabel: desktopAgent.agentLabelId,
...
async function installService(
  ...
  if (ownership.kind === "smappservice") {
    throw cliError({
      code: CLI_ERROR_CODES.SERVICE_INSTALL_FAILED,
      message: `service install: label '${options.label.id}' is owned by SMAppService (loaded from ${ownership.path}); the CLI must not bootout/bootstrap this label. If the Desktop-managed host is broken, run 'hukum host service uninstall' to remove its registration and re-run this command; otherwise relaunch the Hukum app to let it repair its own host.`,
      details: {
  ...
  if (agentOwnership.kind === "smappservice") {
    throw cliError({
      code: CLI_ERROR_CODES.SERVICE_INSTALL_FAILED,
      message: `service install: Hukum Desktop owns host registration on this machine (SMAppService agent '${agentLabelId}' loaded from ${agentOwnership.path}); installing the raw '${options.label.id}' LaunchAgent would run a second host beside it. Re-run with --takeover to move host management to the CLI (the running host is stopped cooperatively first), or relaunch the Hukum app to let it repair its own host.`,
      details: {
        label: options.label.id,
  ...
  try {
    await run("launchctl", ["kickstart", `${guiTarget}/${options.label.id}`], {
      env: undefined,
      cwd: undefined,
      // 30s, not 10s: the dev wrapper at ~/.hukum/cli/dev/bin/hukum
...
async function reloadRegisteredService(
  ...
  try {
    ...
    throw cliError({
      code: CLI_ERROR_CODES.SERVICE_INSTALL_FAILED,
      message: `launchctl bootstrap failed for ${options.labelId} after reload retry; the previous registration was booted out, so the service is now unregistered until 'hukum host service install' succeeds: ${describeCause(cause)}`,
      details: {
        label: options.labelId,
        cause: describeCause(cause),
      },
...
async function uninstallService(
  ...
  if (ownership.kind === "smappservice") {
    createCliLogger(options.label.environment).warn(
      "Service uninstall: label is registered by Hukum Desktop's login item (SMAppService); booting it out now, but macOS may keep the login-item record. If the host reappears at next login, remove Hukum in the Desktop app or System Settings -> Login Items.",
  ...
  if (agentOwnership.kind === "smappservice") {
    createCliLogger(options.label.environment).warn(
      "Service uninstall: Hukum Desktop's SMAppService agent is registered for this environment; booting it out now, but macOS may keep the login-item record. If the host reappears at next login, remove Hukum in the Desktop app or System Settings -> Login Items.",
...
async function statusService(
  ...
  // here used to make every `hukum login` on a Desktop-managed machine
...
// Who owns the host on this machine: Hukum Desktop's post-label-split
...
// or LWCR mutation). The old behavior - refusing with "use the Hukum app" -
...
async function stopDesktopManagedHost(
  ...
  switch (outcome.kind) {
    ...
    case "unreachable":
      throw cliError({
        code: CLI_ERROR_CODES.SERVICE_CONTROL_FAILED,
        message: `host stop: Hukum Desktop owns host registration on this machine (SMAppService agent '${agent.agentLabelId}' loaded from ${agent.loadedPath}) and the running host's RPC endpoint is unreachable (${outcome.cause}); stopping it from the CLI would deregister Desktop's agent. Use the Hukum app to stop it, or take over management with 'hukum host service uninstall' followed by 'hukum host service install', then retry.`,
        details: {
          label: label.id,
          agentLabel: agent.agentLabelId,
...
async function retireCompetingRegistration(
  ...
  if (agentWedge !== "not-wedged") {
    createCliLogger(label.environment).warn(
      "Service repair: the Hukum Desktop agent may not be spawnable, so the competing CLI registration was kept - it may be the only host that starts on this machine.",
  ...
  if (ownership === null) {
    ...
    bootoutFailed = true;
    logger.warn(
      "Service repair: could not read who owns the competing CLI label, so it was not evicted; if the host is unreachable, open Hukum or log out and back in.",
    ...
    try {
      ...
      if (!isBenignBootoutFailure(cause)) {
        ...
        logger.warn(
          "Service repair: could not confirm the competing CLI-label host was evicted; if the host is unreachable, open Hukum or log out and back in.",
          {
            label: label.id,
            agentLabel: agentLabelId,
            cause: describeCause(cause),
  ...
  if (bootedOut) {
    ...
    logger.info(
      "Service repair: evicted the competing CLI-label host - Hukum Desktop's SMAppService agent owns the host on this machine.",
      { label: label.id, agentLabel: agentLabelId },
    );
  }
  ...
  if (bootedOut) {
    try {
      ...
      logger.warn(
        "Service repair: evicted the competing CLI-label host but could not start Hukum Desktop's agent; open Hukum or log out and back in if the host is unreachable.",
        {
          label: label.id,
          agentLabel: agentLabelId,
          cause: describeCause(cause),
...
// Hukum app in System Settings → Login Items (macOS 13+; older releases
...
// the key is inert.
const DESKTOP_APP_BUNDLE_ID = "ai.hukum.desktop";
 ...
 * user's shell (e.g. `make install-desktop-*`, `hukum host install`),
 ...
 * once ran Desktop's setup has a stale staged `~/.hukum/cli` binary that
 ...
 * launcher path, not merely a `hukum-host-start` basename suffix, since a

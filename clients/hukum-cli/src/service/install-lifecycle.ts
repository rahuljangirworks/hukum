...
// extra `hukum host service install` step. `host update` leaves it
  ...
  const lifecycle: InstallHostLifecycle = {
    ...
    afterSwap: async () => {
      if (state.priorState === "externally-managed") {
        // Hukum Desktop's SMAppService owns registration here. Any
      ...
      if (state.priorState === "not-installed") {
        if (options.bootstrap === null) {
          ...
          // operator (`hukum host service install`).
          state.postSwapAction = "none";
          return;
        }
        ...
        try {
          ...
          // `hukum host doctor` / `hukum host service install`
          // for recovery.
          state.postSwapError =
            cause instanceof Error ? cause.message : String(cause);
        }
      ...
      try {
        ...
        // prefer a stale staged `~/.hukum/cli` binary over the brew /
        ...
        await registerService({
          ...
          // re-register on installs that never staged ~/.hukum/cli.
          bootstrap: options.bootstrap ?? {
            enableLinger: false,
...
async function registerService(opts: RegisterServiceOptions): Promise<void> {
  ...
  // host install. Doctor + `hukum host service install` are the recovery
  // paths.
  const cli =
    opts.preservedCli ??

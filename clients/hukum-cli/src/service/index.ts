...
// SMAppService in-bundle plist - Hukum Desktop owns the registration, not
...
export function formatServiceLifecycleWarning(
  ...
  return `warning: service ${action} failed: ${error} - run 'hukum host doctor'`;
}
...
async function announceStop(
  ...
  throw cliError({
    ...
    message: `could not record the stop request, and on Windows that record is the only thing that stops the supervisor bringing the host back. ${FAILED_STOP_CONSEQUENCE[reason]} Check that the Hukum host directory is writable, then try again.`,

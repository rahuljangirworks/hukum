...
import { HOST_START_LAUNCHER_BASENAME } from "@hukum-clients/shared/host-lifecycle";
...
// (`ai.hukum.host.<environment>`, with production keeping the bare
// `ai.hukum.host`), so each channel owns an isolated slot and `environment`
...
// (`ai.hukum.host.<environment>`), mirroring the per-environment install
// tree (`~/.hukum/<component>/<environment>/`). Collapsing them onto a
...
export function serviceLabelFor(environment: Environment): ServiceLabel {
  ...
  return {
    ...
    displayName: `Hukum Host (${capitalizeEnvironment(environment)})`,
    environment,
    devSlot: null,
  };
}

// The label Hukum Desktop registers via SMAppService for the same
...
// (`~/.hukum/service/<label-id>/hukum-host-start`). The label id is the
// parent directory - load-bearing, not tidiness: the shared recognizer
// (`attestHukumRegistration`) matches a launcher-form plist to a label by
...
// Windows Scheduled Task identifier. production = `\Hukum\Host`,
// non-production = `\Hukum\Host-<Environment>` (e.g. `Host-Dev`,
// `Host-Staging`).
export function windowsTaskName(label: ServiceLabel): string {
  if (label.environment === "production") return "\\Hukum\\Host";
  if (label.devSlot !== null) {
    return `\\Hukum\\Host-Dev-${capitalizeEnvironment(label.devSlot)}`;
  }
  return `\\Hukum\\Host-${capitalizeEnvironment(label.environment)}`;
}

...
//      truth when present.
//   2. Install-scoped bin dir at `<cliInstallHomeDir>/bin/hukum` - the
//      well-known staging location every install path drops a binary
//      (or wrapper script) into BEFORE invoking `hukum host
//      install`. Used by:
//        - Desktop's setup splash (stages bundled CLI at
//          `~/.hukum/cli/bin/hukum` before host-bootstrap)
//        - The dev orchestrator (`scripts/dev-desktop.js` stages a
//          bun wrapper at `~/.hukum/cli/dev-runs/<slot>/bin/hukum`

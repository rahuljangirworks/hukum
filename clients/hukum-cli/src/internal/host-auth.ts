 ...
 * with a bearer it reads from `~/.hukum/cli/credentials` - the single,
 * durable source seeded by `hukum login` (the Desktop runs
 * `hukum login --token` after sign-in). The credentials file is the sole
 ...
 * callers can surface a "run `hukum login`" error rather than dialing the

/**
 * Parses the `--profile <ambient|id>` option shared by `hukum agent create`,
...
export function parseConcreteProfileSelection(
  ...
  if (value.length === 0) {
    throw cliError({
      ...
      message: `hukum: --profile must be '${AMBIENT_PROFILE_ID_SENTINEL}' or a managed profile id - run 'hukum agent list-profiles <harness>' to see the available values.`,
      details: null,
      exitCode: 1,
    });
  }

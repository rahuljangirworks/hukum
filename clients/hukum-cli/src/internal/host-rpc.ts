 ...
 * credentials (`resolveHostAuth`), seeded by `hukum login`; on a host
...
export async function resolveEndpoint(): Promise<HostTransportEndpoint> {
  ...
  if (metadata === null) {
    ...
    throw cliError({
      ...
      message:
        "hukum: host not running - pid metadata is absent, malformed, advertises an invalid local WebSocket endpoint, or names a process that is no longer alive.",
      details: null,
      exitCode: 1,
    });
  }
...
export function parseHostResponse<T>(schema: ZodType<T>, value: unknown): T {
  ...
  throw cliError({
    ...
    message:
      "hukum: the host returned a response this CLI could not parse (possible host/CLI version mismatch) - try 'hukum host restart'.",
    details: null,
    exitCode: 1,
  });
}
...
function mapHostRpcError(err: HostRpcError): CliError {
  if (err.code === "FORBIDDEN" || isAccessDenied(err)) {
    return cliError({
      ...
      message:
        "hukum: access denied for this epic - check HUKUM_EPIC_ID and that you're signed in to the account that owns it.",
      details: null,
      exitCode: 1,
    });
  }
  if (err.code === "E_AGENT_NOT_FOUND") {
    return cliError({
      ...
      message: `hukum: ${err.message} Check --agent-id (or $HUKUM_AGENT_ID).`,
      details: null,
      exitCode: 1,
    });
  }
  ...
  if (err.code === "INCOMPATIBLE" || err.code === "DOWNGRADE_UNSUPPORTED") {
    ...
    // form (with the exact per-install-vector command) lives in `hukum
    ...
    // SAME verdict `hukum host doctor` derives - instead of falling through
    // null guidance to an ineffective "restart" hint.
    return cliError({
      code: CLI_ERROR_CODES.HOST_INCOMPATIBLE,
      message: `hukum: ${err.message} - ${compatRecoveryHint(effectiveUpgradeGuidance(err.code, err.fatalDetails?.upgradeGuidance ?? null))}.`,
      details: null,
      exitCode: 1,
    });
  }

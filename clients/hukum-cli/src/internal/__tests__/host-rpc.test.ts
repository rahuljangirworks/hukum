...
import type { CredentialsMutationStore } from "@hukum/protocol/config/credentials-mutation";
...
describe("callHostRpc", () => {
  ...
  it("maps per-feature host unsupported errors distinctly from incompatibility", async () => {
    await expect(
      ...
      message:
        "hukum: This host does not support 'agent.future'. Upgrade the host to use this feature.",
      details: {
  ...
  it("maps oversized agent messages to invalid argument", async () => {
    await expect(
      ...
      message: "hukum: Message exceeds the maximum size.",
      details: null,
    });
  });
});

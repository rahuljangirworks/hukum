...
beforeAll(() => {
  tmpRoot = mkdtempSync(join(tmpdir(), "hukum-minisign-test-"));
});
...
describe("verifyMinisignArchive", () => {
  it("accepts a valid pre-hashed signature", async () => {
    ...
    const signatureText = makeSignatureFile({
      archivePath,
      key,
      trustedComment: "hukum-host 1.0.0 darwin-arm64",
      prehashed: true,
    });
    ...
    expect(result.trustedComment).toBe("hukum-host 1.0.0 darwin-arm64");
  });

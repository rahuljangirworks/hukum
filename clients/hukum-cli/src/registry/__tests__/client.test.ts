...
// a test run never touches the developer's real ~/.hukum.
...
beforeAll(() => {
  tmpRoot = mkdtempSync(join(tmpdir(), "hukum-registry-client-"));
});
...
describe("registry client", () => {
  ...
  it("refuses a version whose client floor this CLI is below", async () => {
    vi.stubEnv("HUKUM_CLI_VERSION", "1.1.8");
    const client = await createRegistryClient({
      environment: "production",
  ...
  it("resolves the same version once the CLI meets the floor", async () => {
    vi.stubEnv("HUKUM_CLI_VERSION", "2.0.0");
    const client = await createRegistryClient({
      environment: "production",
      transport: flooredTransport(),
      onProgress: null,

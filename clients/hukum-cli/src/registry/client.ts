...
export async function createRegistryClient(
  ...
  if (trustedKeySet.keys.length === 0 && opts.requireTrustedKeys) {
    ...
    throw cliError({
      ...
      message:
        ...
        "builds; install from a local archive with 'hukum host install --from <path>'.",
      details: {
  ...
  return {
    async fetchManifest(): Promise<HostVersionsManifest> {
      ...
      let body: string;
      try {
        body = await transport.fetchText({
    ...
    async downloadAndVerify(
      ...
      // point of hukum#585/#588. `acquireDownloadSlot` owns the

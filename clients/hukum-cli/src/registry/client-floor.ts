...
export function hostClientFloorRefusalMessage(
  ...
  if (refusal.kind === "below-floor") {
    return `host registry: version '${resolvedVersion}' requires Hukum CLI ${refusal.requiredCliVersion} or newer, and this is ${refusal.cliVersion}. Update Hukum first, then install the host.`;
  }

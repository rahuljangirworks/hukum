...
describe("acquireCliLock", () => {
  ...
  beforeEach(() => {
    workDir = mkdtempSync(join(tmpdir(), "hukum-cli-lock-test-"));
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const childA = spawnWorker("A", {
        HUKUM_CLI_LOCK_TEST_BREAK_HOOK_DIR: hookDir,
      });
...
describe("release() compare-and-delete", () => {
  ...
  beforeEach(() => {
    workDir = mkdtempSync(join(tmpdir(), "hukum-cli-lock-release-test-"));
    mocks.lockPath = join(workDir, ".lock");
  });

...
// `@hukum-clients/shared/host-lock/process-identity` alongside the
...
// `@hukum-clients/shared/host-lock/cross-process-lock` (Host Update
...
function lockBusyError(path: string, holder: LockMetadata | null): Error {
  return cliError({
    ...
    message:
      holder === null
        ? `another hukum CLI mutation is in progress (lock=${path})`
        : `another hukum CLI mutation is in progress (lock=${path}, holder.pid=${holder.pid}, reason=${holder.reason}, since=${holder.startedAt})`,
    details: {
      lockPath: path,
      holder,
    },
...
// bypassing `cliLockPath`'s real `~/.hukum` resolution entirely. The
...
export async function withCliLock<T>(
  ...
  const outcome = await withLock(
    ...
    async (handle) => {
      ...
      const acquiredMarker = process.env.HUKUM_CLI_LOCK_ACQUIRED_MARKER;

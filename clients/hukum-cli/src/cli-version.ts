...
// CI release workflows set `HUKUM_CLI_VERSION` from the `cli-v<version>`
// tag, and `build-cli-sea.cjs` bakes that value into the bundle via an
// esbuild define - when that path runs, `process.env.HUKUM_CLI_VERSION`
 ...
 * `process.env.HUKUM_CLI_VERSION`; everything else (tsx dev, vitest,
...
export function resolveCliVersion(
  ...
  const injected = env.HUKUM_CLI_VERSION;

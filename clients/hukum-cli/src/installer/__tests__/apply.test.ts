...
// would otherwise resolve against the REAL production `~/.hukum`, not
...
async function writeInstall(
  ...
  const executablePath = join(installDir, "hukum-host");
...
async function writeStaged(
  ...
  const executableRelPath = "hukum-host";
...
describe("applyHost", () => {
  beforeEach(() => {
    sandboxRoot = mkdtempSync(join(tmpdir(), "hukum-apply-test-"));

...
// `~/.hukum`, not this sandbox - confirmed root cause of a real prod-host
...
function sampleInstallRecordJson(version: string): Record<string, unknown> {
  return {
    ...
    sizeBytes: 1,
    executablePath: join(installDirFor(ENV), "hukum-host"),
  };
}

describe("uninstallHost", () => {
  beforeEach(() => {
    sandboxRoot = mkdtempSync(join(tmpdir(), "hukum-uninstall-test-"));
  ...
  it("removes both install/ and staged/ from disk, alongside the install record", async () => {
    ...
    writeFileSync(join(installDir, "hukum-host"), "binary");
    writeFileSync(

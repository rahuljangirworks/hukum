...
beforeAll(() => {
  scratchRoot = mkdtempSync(join(tmpdir(), "hukum-cli-extract-"));
});
...
describe("extractHostSource (tar)", () => {
  ...
  it("tolerates plain leading-dot entries (e.g. .hukum-marker)", async () => {
    const sourceDir = mkdtempSync(join(scratchRoot, "src-"));
...
describe("extractHostSource (onEntry liveness)", () => {
  ...
  it("fires while copying a bare executable", async () => {
    const sourceDir = mkdtempSync(join(scratchRoot, "src-"));
    const source = join(sourceDir, "hukum-host");
    ...
    expect(ticks).toBe(1);
    expect(await readFile(join(targetDir, "hukum-host"), "utf8")).toBe(
      "binary\n",
    );
  });
});

...
function executableBasename(): string {
  return osPlatform() === "win32" ? "hukum-host.exe" : "hukum-host";
}
...
describe("downloadAndStageHost", () => {
  beforeEach(() => {
    sandboxRoot = mkdtempSync(join(tmpdir(), "hukum-download-stage-test-"));
    osHome.current = sandboxRoot;
    archiveTmpDir = mkdtempSync(
      join(tmpdir(), "hukum-download-stage-archives-"),
    );

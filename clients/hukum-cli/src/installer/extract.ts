...
export async function resolveHostExecutable(
  ...
  // Production ships a real `hukum-host.exe` SEA binary; the `make
  // dev-desktop` orchestrator stages a `hukum-host.cmd` wrapper that execs
  ...
  // first.
  const expectedNames = platform === "win32"
      ? ["hukum-host.exe", "hukum-host.cmd", "hukum-host.bat"]
      : ["hukum-host"];

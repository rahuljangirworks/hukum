    ...
    async () => {
      ...
      if (installed === null) {
        throw cliError({
          ...
          message: `host download: no host installed for environment=${environment}; run 'hukum host install latest' first`,
          details: { environment },
          exitCode: 1,
        });
      }
  ...
  const preDownload = await withCliLock(
    ...
    async () => {
      ...
      if (installed === null) {
        throw cliError({
          code: CLI_ERROR_CODES.HOST_NOT_INSTALLED,
          message: `host download: no host installed for environment=${opts.environment}; run 'hukum host install latest' first`,

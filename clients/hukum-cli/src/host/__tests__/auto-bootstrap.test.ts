...
beforeEach(() => {
  ...
  serviceLabelForMock.mockImplementation(
    (environment: "production" | "dev") => ({
      id: environment === "dev" ? "ai.hukum.host.dev" : "ai.hukum.host",
...
describe("evaluateAutoBootstrap", () => {
  ...
  it("returns ready (never 'service repair') when the service is externally managed by Desktop's SMAppService", async () => {
    ...
    // every `hukum login`, which then died on installService's
  ...
  it("respects HUKUM_NONINTERACTIVE / CI when host is installed but service is missing", async () => {
    readHostInstallRecordMock.mockReturnValue({ version: "1.5.0" });
    const fake = makeFakeServiceController({
      state: "not-installed",
      installCalls: 0,

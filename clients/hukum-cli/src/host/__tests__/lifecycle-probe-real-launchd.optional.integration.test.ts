 ...
 * scoped labels (`ai.hukum.host.test.<pid>.<runId>[...]`), scoped temp
 * dirs, macOS-only, manual/opt-in (never default PR CI).
 */

const ENABLED = process.env.HUKUM_RUN_REAL_SUPERVISOR_MACOS === "1";
 ...
 * - a job that declares `HUKUM_REQUIRE_LAYER0=1` — the internal monorepo's
 *   `real-supervisor-internal.yml`, where `hukum-host/` exists — **fails**
...
const REQUIRE_LAYER0 = process.env.HUKUM_REQUIRE_LAYER0 === "1";
...
async function assertRealLaunchdReady(): Promise<void> {
  ...
  if (process.platform !== "darwin") {
    problems.push(
      `platform is "${process.platform}", but these rows drive launchd and ` +
        'require "darwin". Unset HUKUM_RUN_REAL_SUPERVISOR_MACOS to skip ' +
    ...
    if (LAYER0_MISSING && REQUIRE_LAYER0) {
      problems.push(
        "HUKUM_REQUIRE_LAYER0=1 says this checkout must be able to contend " +
    ...
    if (LAYER0_MISSING && !REQUIRE_LAYER0) {
      ...
      process.stderr.write(
        "\n[real-launchd] Layer-0 rows NOT EXECUTED: this is an OSS " +
          ...
          "sets HUKUM_REQUIRE_LAYER0=1 so their absence fails there.\n\n",
      );
    }
  }
  if (problems.length > 0) {
    throw new Error(
      [
        "HUKUM_RUN_REAL_SUPERVISOR_MACOS=1 is set, but the real-launchd " +
...
describe.skipIf(!ENABLED)(
  ...
  () => {
      ...
      async () => {
        await withScopedSupervisorProbe(
          ...
          async ({ label, marker }) => {
            ...
            expect(label).toContain("ai.hukum.host.test.");
          },
        );
      },
      15_000,
    ...
    it("M9: an unloaded (never-bootstrapped) label attests to null, never throws", async () => {
      ...
      const label = `ai.hukum.host.test.${process.pid}.${runId}.never-loaded`;
      ...
      async () => {
        await withScopedFallbackReadiness(
          async ({
            ...
            expect(hostRoot).toContain(".hukum/host/dev");
...
async function withScopedFallbackReadiness(
  ...
  const runId = randomUUID().replaceAll("-", "");
  const label = `ai.hukum.host.test.${process.pid}.${runId}.fallback`;
  ...
  const hostRoot = join(home, ".hukum", "host", "dev");
...
async function withScopedSupervisorProbe(
  ...
  const uid = process.getuid?.() ?? 0;
  const runId = randomUUID().replaceAll("-", "");
  const label = `ai.hukum.host.test.${process.pid}.${runId}.supervisor`;
  const argvLabel =
...
/**
 * The Layer-0 lock lives in `hukum-host`, which is **not part of the OSS
...
function layer0ModulePath(): string {
  return resolve(
    process.cwd(),
    "../../../hukum-host/src/lifecycle/layer0-lock.ts",
  );
}
...
function layer0AddonPath(): string {
  return resolve(
    process.cwd(),
    "../../../hukum-host/native/lifecycle-lock/build/Release/lifecycle_lock.node",
  );
}
  ...
  if (!existsSync(layer0AddonPath())) {
    return {
      ...
      reason:
        ...
        "Run `node hukum-host/scripts/build-lifecycle-lock.cjs`. Skipping " +
...
async function withScopedJob(
  ...
  const uid = process.getuid?.() ?? 0;
  const runId = randomUUID().replaceAll("-", "");
  const label = `ai.hukum.host.test.${process.pid}.${runId}`;

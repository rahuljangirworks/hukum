...
function hostHomeFor(environment: Environment): string {
  const hostRoot = join(sandboxRoot, ".hukum", "host");
...
function cliLockPathFor(environment: Environment): string {
  const cliRoot = join(sandboxRoot, ".hukum", "cli");
  const cliHome =
...
// `~/.hukum`, not this sandbox. Redirect the `os` boundary itself so
...
function writeLocalHostSource(path: string): void {
  ...
  writeFileSync(join(path, "hukum-host"), "test host binary");
}

describe("stampRuntime", () => {
  beforeEach(() => {
    sandboxRoot = mkdtempSync(join(tmpdir(), "hukum-stamp-runtime-test-"));
  ...
  () => {
    ...
    beforeEach(() => {
      sandboxRoot = mkdtempSync(
        join(tmpdir(), "hukum-stamp-runtime-live-test-"),
      );
      osHome.current = sandboxRoot;
    });
...
describe.skipIf(process.platform === "win32")(
  ...
  () => {
    beforeEach(() => {
      sandboxRoot = mkdtempSync(
        join(tmpdir(), "hukum-stamp-runtime-cas-race-test-"),
      );
      osHome.current = sandboxRoot;
    });
    ...
    it("a real terminal install landing in a separate OS process between the attested generation and the stamp call is superseded, never stamped onto the interloper's record", async () => {
      ...
      mkdirSync(join(sandboxRoot, ".hukum", "cli", ENV), {
        recursive: true,
      });

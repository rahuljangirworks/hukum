...
// Redirect host.log for StderrLogTee so tests never write under ~/.hukum.
...
// ---------------------------------------------------------------------------
// StderrLogTee (mocked path so it never touches ~/.hukum)
// ---------------------------------------------------------------------------
...
describe("StderrLogTee", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "hukum-stderr-tee-"));
...
describe("prepareCrashReportsDir", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "hukum-crash-prep-"));
  });
...
describe("pruneCrashReports", () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), "hukum-crash-prune-"));
  });
...
describe("findCrashReportSince", () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), "hukum-crash-find-"));
  });

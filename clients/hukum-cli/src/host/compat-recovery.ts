...
// caller can act on:
//
//   hostShouldUpgrade → reinstall the latest host (`hukum host update`).
...
export const CLIENT_UPGRADE_HINT_FOR_SOURCE: Record<CliInstallSource, string> =
  {
    desktop:
      "Update the Hukum desktop app (it updates itself via its built-in updater), then relaunch.",
    manual:
      "Run 'npm update -g @hukumai/cli' if you installed via npm, otherwise re-download the latest standalone CLI.",
    ...PACKAGE_MANAGER_UPGRADE_HINT,
  };
...
function summarize(
  ...
  if (reinstallHost && clientUpgrade !== null) {
    return `The host and this client are both out of date. Reinstall the latest host ('hukum host update'), then update the client: ${clientUpgrade.hint}`;
  }
  if (reinstallHost) {
    return "The host is out of date. Reinstall the latest host with 'hukum host update'.";
  }
  if (clientUpgrade !== null) {
    return `This Hukum client is out of date. ${clientUpgrade.hint}`;
  }
  ...
  return "Restart the host ('hukum host restart'); if the mismatch persists, update both the host and this client.";
}
...
export function compatRecoveryHint(
  ...
  if (host && client) {
    return "both the host and this CLI are out of date - run 'hukum host update' and update the CLI via your install method";
  }
  if (host) {
    return "the host is out of date - run 'hukum host update' to reinstall the latest host";
  }
  if (client) {
    return "this CLI is out of date - update it via your install method (e.g. 'hukum cli upgrade', 'brew upgrade hukum', or 'npm update -g @hukumai/cli')";
  }
  return "try 'hukum host restart'; if it persists, update the host and CLI";
}

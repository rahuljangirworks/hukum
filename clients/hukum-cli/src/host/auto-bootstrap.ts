...
// (Standalone CLI First-Run), commands like `hukum login` and any
...
export async function detectBootstrapState(runtime: RuntimeContext): Promise<{
  ...
  try {
    ...
    // Treating it as unregistered used to send every `hukum login` on a
...
// `hukum login` / `host status` bootstrap a self-contained, offline host.

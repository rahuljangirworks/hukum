...
// post-install scripts drop /var/lib/hukum/source.{apt,rpm} so the
// CLI knows the binary is package-manager-owned even before the
// per-user manifest at ~/.hukum/cli/manifest.json has been written.
//
...
// reader probes the test workspace rather than the real ~/.hukum.
// `store/paths` computes `HUKUM_HOME` from `os.homedir()` once at module
// load - any export this mock leaves un-overridden would otherwise resolve
// against the REAL production `~/.hukum`, not this sandbox. Redirect the
...
describe("readCliManifest - system-marker fallback (Linux .deb / .rpm)", () => {
  ...
  beforeEach(() => {
    sandboxRoot = mkdtempSync(join(tmpdir(), "hukum-cli-marker-test-"));
    osHome.current = sandboxRoot;
    markerDir = join(sandboxRoot, "var-lib-hukum");
  ...
  it("Linux + prod environment + apt marker present: synthesises an 'apt' manifest", async () => {
    ...
    expect(result?.binaryPath).toBe("/usr/bin/hukum");
  ...
  it("npm distribution stamp synthesises an npm-owned manifest when no per-user manifest exists", async () => {
    const previousDistribution = process.env.HUKUM_CLI_DISTRIBUTION;
    const previousVersion = process.env.HUKUM_CLI_VERSION;
    const previousArgv1 = process.argv[1];
    process.env.HUKUM_CLI_DISTRIBUTION = "npm";
    process.env.HUKUM_CLI_VERSION = "1.2.3";
    process.argv[1] = "/usr/local/bin/hukum";
    try {
      ...
      if (previousDistribution === undefined) {
        delete process.env.HUKUM_CLI_DISTRIBUTION;
      } else {
        process.env.HUKUM_CLI_DISTRIBUTION = previousDistribution;
      }
      if (previousVersion === undefined) {
        delete process.env.HUKUM_CLI_VERSION;
      } else {
        process.env.HUKUM_CLI_VERSION = previousVersion;
      }
  ...
  it("Linux + prod environment + marker with missing version field: returns null (best-effort parse)", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ binaryPath: "/usr/bin/hukum" }),
      "utf8",
    );
    const result = await withPlatform("linux", () =>
      readCliManifest("production"),

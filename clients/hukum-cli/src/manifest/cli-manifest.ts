...
// ~/.hukum/cli/manifest.json for prod, ~/.hukum/cli/dev/manifest.json for
// legacy/no-slot dev, and ~/.hukum/cli/dev-runs/<slot>/manifest.json for
...
export const PACKAGE_MANAGER_UPGRADE_HINT: Record<
  ...
  string
> = {
  homebrew: "Run 'brew upgrade hukum'.",
  npm: "Run 'npm install -g @hukumai/cli@latest'.",
  winget: "Run 'winget upgrade Hukum.CLI'.",
  scoop: "Run 'scoop update hukum-cli'.",
  apt: "Run 'sudo apt update && sudo apt install --only-upgrade hukum-cli'.",
  rpm: "Run 'sudo dnf upgrade hukum-cli' (or 'yum upgrade').",
};
...
function readDistributionInstallSourceFromEnv(): CliInstallSource | null {
  const value = process.env.HUKUM_CLI_DISTRIBUTION;
...
// manifest at ~/.hukum/cli/manifest.json hasn't been written yet
// (e.g. first invocation after an unattended apt install).
const DEFAULT_SYSTEM_SOURCE_MARKER_DIR = "/var/lib/hukum";
...
// without touching `/var/lib/hukum`. Production code never calls
...
// Read /var/lib/hukum/source.{apt,rpm} if present. Best-effort:
...
// (/var/lib/hukum/source.{apt,rpm}) is present, synthesize a manifest

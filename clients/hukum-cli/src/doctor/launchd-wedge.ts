    ...
    return {
      ...
      message: desktopOwned
        ? `launchd has '${labelId}' loaded but the job cannot run (${summary}). ` +
          ...
          "Hukum Desktop app to repair it; if that does not help, run " +
          "'hukum host service uninstall' and then relaunch the app (or run " +
          "'hukum host service install' to switch to CLI management)."
        : `launchd has '${labelId}' loaded but the job cannot run (${summary}). ` +
          "Run 'hukum host service install' to re-register it. A restart " +
      ...
      terminalCommand: desktopOwned
        ? "hukum host service uninstall"
        : "hukum host service install",
      details: {
        labelId,

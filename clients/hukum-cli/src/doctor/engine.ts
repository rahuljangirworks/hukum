...
export async function runDoctor(opts: RunDoctorOptions): Promise<DoctorResult> {
  ...
  try {
    ...
    if (err instanceof CliError) {
      issues.push({
        ...
        terminalCommand: `hukum host install latest`,
        details: err.details,
      });
      record = null;
    } else {
  ...
  try {
    ...
    issues.push({
      ...
      terminalCommand: `hukum host service install`,
  ...
  if (serviceStatus !== null) {
    if (serviceStatus.state === "not-installed") {
      ...
      issues.push({
        ...
        severity: "info",
        title: "Service managed by Hukum Desktop",
        message: `The OS service for '${label.id}' is registered by the Hukum Desktop app (SMAppService login item); the CLI manages the host's lifecycle cooperatively but not its registration. Use the Hukum app to repair or remove the host on this machine. If the app itself is the broken part, take registration over from the CLI with 'hukum host service install --takeover'.`,
        ...
        fixAction: null,
        terminalCommand: `hukum host service install --takeover`,
        details: { label: label.id },
      });
      ...
      // card) MUST NOT be `hukum host start`, though: that
    ...
    if (!reachable) {
      ...
      if (conflict !== null && portInfo !== null) {
        ...
        issues.push({
          ...
          terminalCommand: `hukum host free-port-and-restart --pid ${conflict.pid} --port ${portInfo.port}`,
          details: {
            pid: pidMetadata.pid,
    ...
    if (!stagedExists) {
      ...
      issues.push({
        ...
        message:
          ...
          "is no longer on disk. Re-run 'hukum cli upgrade' to re-stage.",
        fixAction: null,
        terminalCommand: `hukum cli upgrade`,
        details: {
  ...
  if (recentCrash !== null) {
    ...
    issues.push({
      ...
      fixAction: "host-logs",
      terminalCommand: `hukum host logs --tail 200`,
      details: {
...
async function probeWindowsCredentialsAcl(
  ...
  //   C:\Users\me\.hukum\cli\credentials NT AUTHORITY\SYSTEM:(F)
...
// `host-install-latest` (reinstall the latest host, `hukum host
...
// the fix is the renderer's existing `host-install-latest` (`hukum host
// update`), and any client-side staleness rides along as copy in the summary
// (no auto-fix button - the CLI must not self-replace a package-manager-owned
// binary). A client-only verdict offers no button; a no-stale-side verdict
  ...
  try {
    ...
    const repairAdvice =
      manifest.source === "desktop"
        ? "Launch the Hukum Desktop app to repair it (the app re-stages its bundled CLI at startup); if the app has been uninstalled, reinstall it, or remove the link and reinstall the CLI another way."

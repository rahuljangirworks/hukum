...
// `hukum host start --environment …` as both the GUI `fixAction` and
...
// copyable `terminalCommand` to `hukum host restart …`, the
...
beforeEach(() => {
  workHome = mkdtempSync(join(tmpdir(), "hukum-doctor-stopped-test-"));
...
describe("runDoctor SERVICE_STOPPED recovery routing", () => {
  it("emits SERVICE_STOPPED with the safe `host restart` terminal command", async () => {
    ...
    expect(issue?.terminalCommand).not.toMatch(/^hukum host start\b/);
    ...
    expect(issue?.details).toMatchObject({
      label: "ai.hukum.host.production",
    });
  });
  ...
  it("surfaces the active environment's service label in the issue details", async () => {
    ...
    expect(issue?.terminalCommand).toBe("hukum host restart");
    expect(issue?.details).toMatchObject({ label: "ai.hukum.host.dev" });
  });
  ...
  it("emits an info-only SERVICE_EXTERNALLY_MANAGED card (no fix) for a Desktop/SMAppService-owned label instead of the not-registered error", async () => {
    ...
    // suggested fix - `hukum host service install` - refuses
    ...
    expect(issue?.terminalCommand).toBe(
      "hukum host service install --takeover",
    );
    expect(issue?.message).toContain("hukum host service install --takeover");
  });
  ...
  it("still emits SERVICE_NOT_REGISTERED when pid metadata proves a host process is running", async () => {
    ...
    expect(issue).toBeDefined();
    expect(issue?.terminalCommand).toBe("hukum host service install");
  });

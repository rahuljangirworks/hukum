...
beforeEach(() => {
  workHome = mkdtempSync(join(tmpdir(), "hukum-doctor-wedge-test-"));
  ...
  () => {
    it("surfaces the wedge probe's issue in the doctor result", async () => {
      ...
      vi.doMock("../launchd-wedge", () => ({
        ...
        probeMacosWedgedJob: async (input: unknown) => {
          ...
          return {
            ...
            fixAction: null,
            terminalCommand: "hukum host service uninstall",
            details: { labelId: "ai.hukum.host.production.agent" },
          };
        },
      }));
      ...
      expect(probeCalls[0]).toMatchObject({
        labelId: "ai.hukum.host.production",
        agentLabelId: "ai.hukum.host.production.agent",

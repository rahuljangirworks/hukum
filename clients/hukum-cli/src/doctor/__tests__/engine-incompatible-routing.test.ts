...
describe("routeIncompatibleRecovery", () => {
  it("host-only verdict → host-install-latest / 'hukum host update'", () => {
    ...
    expect(r.terminalCommand).toBe("hukum host update");
  });
  ...
  it("INCOMPATIBLE with no guidance → conservative host-restart fallback", () => {
    ...
    expect(r.terminalCommand).toBe("hukum host restart");
  });
});

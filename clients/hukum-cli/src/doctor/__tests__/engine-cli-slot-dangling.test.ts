...
// The stable CLI path (`~/.hukum/cli/bin/hukum`) is a symlink the
...
beforeEach(() => {
  workHome = mkdtempSync(join(tmpdir(), "hukum-doctor-dangling-test-"));
...
describe.runIf(process.platform !== "win32")(
  ...
  () => {
    it("names the dangling-symlink state with the app-relaunch repair for a desktop-staged CLI", async () => {
      ...
      const binDir = join(workHome, ".hukum", "cli", "bin");
      mkdirSync(binDir, { recursive: true });
      const slotPath = join(binDir, "hukum");
      const goneTarget = join(workHome, "Hukum.app-removed", "hukum");
    ...
    it("stays silent for a symlink whose target exists", async () => {
      ...
      const binDir = join(workHome, ".hukum", "cli", "bin");
      mkdirSync(binDir, { recursive: true });
      const realBinary = join(workHome, "Hukum.app", "hukum");
      mkdirSync(join(workHome, "Hukum.app"), { recursive: true });

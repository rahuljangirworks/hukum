...
// Pending CLI upgrade is staged by `hukum cli upgrade` when the
...
beforeEach(() => {
  workHome = mkdtempSync(join(tmpdir(), "hukum-pending-upgrade-test-"));
...
function writeManifest(
  ...
  const dir =
    environment === "production"
      ? join(workHome, ".hukum", "cli")
      : join(workHome, ".hukum", "cli", "dev");
...
describe("finalizePendingCliUpgrade", () => {
  ...
  it("reports staged-binary-missing when the staged file is gone", async () => {
    const liveBinaryPath = join(workHome, "bin", "hukum");
    const stagedBinaryPath = join(workHome, "bin", "hukum-staged-missing");
    mkdirSync(join(workHome, "bin"), { recursive: true });
    writeFileSync(liveBinaryPath, "live");
    writeManifest(
      "production",

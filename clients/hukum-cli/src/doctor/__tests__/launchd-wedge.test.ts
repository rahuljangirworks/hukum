...
import type { ProbeCommandResult } from "@hukum-clients/shared/host-lifecycle";
...
describe.runIf(process.platform !== "win32")("probeMacosWedgedJob", () => {
  it("reports a wedged Desktop-owned agent with the takeover escape hatch", async () => {
    ...
    const issue = await probeMacosWedgedJob({
      ...
      agentLabelId: "ai.hukum.host.agent",
    ...
    expect(issue?.message).toContain("ai.hukum.host.agent");
    ...
    expect(issue?.message).toContain("Hukum Desktop app");
    expect(issue?.message).toContain("hukum host service uninstall");
    expect(issue?.terminalCommand).toBe("hukum host service uninstall");
  ...
  it("stays silent on a healthy loaded agent and never probes the second label", async () => {
    ...
    expect(targets[0]).toContain("ai.hukum.host.agent");
  });
  ...
  it("falls through an absent agent label to a wedged CLI-owned job and routes to a real RE-REGISTRATION, never a restart", async () => {
    ...
    expect(issue?.terminalCommand).not.toBe("hukum host restart");

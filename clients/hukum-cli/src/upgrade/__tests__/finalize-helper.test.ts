...
beforeEach(() => {
  workHome = mkdtempSync(join(tmpdir(), "hukum-finalize-helper-test-"));
...
function writeManifest(opts: {
  ...
  const cliDir = join(workHome, ".hukum", "cli");
...
describe("scheduleFinalizationHelper", () => {
  it("renders a PowerShell script with parent pid + paths and spawns powershell.exe detached on Windows", async () => {
    ...
    const result = await scheduleFinalizationHelper({
      ...
      stagedBinaryPath: "C:/Users/dev/AppData/.hukum/cli/hukum-1.5.0.exe",
      livePath: "C:/Users/dev/AppData/.hukum/cli/hukum.exe",
      parentPid: 4242,
    ...
    expect(body).toContain("C:/Users/dev/AppData/.hukum/cli/hukum.exe");
    expect(body).toContain(
      "C:/Users/dev/AppData/.hukum/cli/hukum-1.5.0.exe",
    );
  ...
  it("renders a POSIX shell script with parent pid + paths and spawns /bin/sh detached on linux", async () => {
    ...
    const result = await scheduleFinalizationHelper({
      environment: "production",
      stagedBinaryPath: "/usr/local/share/hukum/cli/hukum-1.5.0",
      livePath: "/usr/local/share/hukum/cli/hukum",
      parentPid: 4242,
      parentExitTimeoutSeconds: 60,
      platform: "linux",
    ...
    expect(body).toContain("/usr/local/share/hukum/cli/hukum");
    // Binary swap + service start hand off to the staged binary's own
...
describe("reconcilePostFinalizeMarker", () => {
  ...
  it("on 'swapped' marker, clears pendingUpgrade, promotes version, and unlinks the marker", async () => {
    const liveBinaryPath = join(workHome, "bin", "hukum");
    const stagedBinaryPath = join(workHome, "bin", "hukum-1.5.0");
    ...
    const markerPath = join(workHome, ".hukum", "cli", "post-finalize.json");
    writeFileSync(
      markerPath,
      JSON.stringify({
        status: "swapped",
  ...
  it("is a no-op when the manifest no longer has a pendingUpgrade (idempotent on repeated apply)", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        binaryPath: join(workHome, "bin", "hukum"),
        source: "manual",
        pendingUpgrade: null,
      }),
      { encoding: "utf8", mode: 0o600 },
    ...
    writeFileSync(
      ...
      JSON.stringify({
        status: "swapped",
        attemptedAt: "2026-05-11T00:00:00Z",
        livePath: join(workHome, "bin", "hukum"),
        stagedBinaryPath: join(workHome, "bin", "hukum-1.5.0"),
        errorMessage: null,
        serviceStartError: null,
      }),
    );

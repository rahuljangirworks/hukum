...
//
// `hukum cli upgrade` stages a new CLI binary on disk and records
...
// install dir is read-only). `hukum host restart` then tries to
...
//      `~/.hukum/cli/post-finalize.json` describing the outcome
...
function makeHelperScriptPath(
  ...
  return join(tmpdir(), `hukum-cli-finalize-${environment}-${stamp}${ext}`);
}
...
function renderWindowsHelperScript(opts: {
  ...
  return `# hukum-cli pending-upgrade finalize helper (Windows)
...
# hukum-cli pending-upgrade finalize helper (POSIX)
set -u

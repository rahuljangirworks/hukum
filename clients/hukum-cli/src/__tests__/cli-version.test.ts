...
// `hukum --version` must report the release-injected
// `HUKUM_CLI_VERSION` for SEA builds and the local/dev fallback
...
describe("resolveCliVersion", () => {
  it("returns HUKUM_CLI_VERSION when the release pipeline injected it", () => {
    expect(resolveCliVersion({ HUKUM_CLI_VERSION: "1.6.0" })).toBe("1.6.0");
  });

  it("preserves pre-release suffixes (e.g. 1.6.0-rc.1) verbatim", () => {
    expect(resolveCliVersion({ HUKUM_CLI_VERSION: "1.6.0-rc.1" })).toBe(
      "1.6.0-rc.1",
    );
  });
  ...
  it("falls back to the local/dev sentinel when the env var is unset", () => {
    ...
    expect(resolveCliVersion({ HUKUM_CLI_VERSION: undefined })).toBe(
      LOCAL_CLI_VERSION,
    );
  });

  it("treats an empty HUKUM_CLI_VERSION as 'unset' so a bogus injection still falls back", () => {
    expect(resolveCliVersion({ HUKUM_CLI_VERSION: "" })).toBe(
      LOCAL_CLI_VERSION,
    );
  });
  ...
  it("SEA release builds overwrite ambient HUKUM_CLI_VERSION with the baked version", () => {
    const buildScript = readFileSync(
      join(__dirname, "..", "..", "scripts", "build-cli-sea.cjs"),
      "utf8",
    );
    expect(buildScript).toContain(
      "process.env.HUKUM_CLI_VERSION=${JSON.stringify(cliVersion)}",
    );
    expect(buildScript).not.toContain(
      "if(!process.env.HUKUM_CLI_VERSION)process.env.HUKUM_CLI_VERSION=",
    );
  });
  ...
  it("npm release builds overwrite ambient CLI version/distribution env with baked package values", () => {
    ...
    expect(buildScript).toContain('process.env.HUKUM_CLI_DISTRIBUTION="npm"');
    expect(buildScript).not.toContain(
      "if(!process.env.HUKUM_CLI_VERSION)process.env.HUKUM_CLI_VERSION=",
    );
    expect(buildScript).not.toContain(
      "if(!process.env.HUKUM_CLI_DISTRIBUTION)process.env.HUKUM_CLI_DISTRIBUTION=",
    );
  });
});

describe("buildProgram() Commander version registration", () => {
  it("registers the local/dev fallback when HUKUM_CLI_VERSION is unset at module-evaluation time", () => {
    // `buildProgram()` reads `process.env.HUKUM_CLI_VERSION` directly,
    ...
    const previous = process.env.HUKUM_CLI_VERSION;
    delete process.env.HUKUM_CLI_VERSION;
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.HUKUM_CLI_VERSION = previous;
      }
    }
  });

  it("registers HUKUM_CLI_VERSION when the release pipeline injected it", () => {
    const previous = process.env.HUKUM_CLI_VERSION;
    process.env.HUKUM_CLI_VERSION = "9.9.9";
    try {
      const program = buildProgram();

...
// `hukum login` (no `--token`) authenticates via the OAuth 2.0 Device
    ...
    for (;;) {
      ...
      if (isDeviceExpired(schedule, Date.now())) {
        throw cliError({
          code: CLI_ERROR_CODES.AUTH_REJECTED,
          message:
            "Sign-in request expired before it was approved - re-run `hukum login` to try again.",
          details: null,
          exitCode: 1,
        });
      }
      ...
      switch (poll.kind) {
        ...
        case "access-denied":
          throw cliError({
            code: CLI_ERROR_CODES.AUTH_REJECTED,
            message: "Sign-in was denied. Re-run `hukum login` to try again.",
            details: null,
            exitCode: 1,
          });
        case "expired":
        ...
        case "invalid":
          throw cliError({
            code: CLI_ERROR_CODES.AUTH_REJECTED,
            message:
              "Sign-in request was rejected. Re-run `hukum login` to try again.",
            details: null,
            exitCode: 1,
          });
      }

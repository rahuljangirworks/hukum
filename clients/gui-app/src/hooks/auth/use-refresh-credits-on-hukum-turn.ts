 ...
 * Hukum-harness chat turn completes - mirroring the extension's
 * `UsageInformationTracker`, which re-fetched the authenticated user after each
 * backend response.
 *
 * Mounted by `HukumSubscriptionSection`, so it runs only while the Hukum
 * credits UI is on screen - the one place the balance is shown, and `"hukum"`
 ...
 */
export function useRefreshCreditsOnHukumTurn(): void {

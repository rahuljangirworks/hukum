/**
 * Host/query-free logic for the signed-in user's Hukum subscription + credits,
 * shared by the Settings › Providers › Hukum card
 * (`hukum-subscription-section.tsx`) and the header rate-limit popover's
 * "Hukum" tab (`rate-limit-popover.tsx`) - mirroring how
...
export type HukumSubscription =
  HukumUserSubscription | HukumTeamSubscription;
...
/**
 * The Hukum plan/tier label for the selected account (Core Flows parity
 * with Codex/Claude: "shown where the provider reports one") - the header
 * popover's "Hukum" tab shows this as a chip next to the name, same as
...
/**
 * Whether the Hukum rail tab should appear at all: the resolved account is on
 * a paid plan, or holds an active credit bundle. Free/pending accounts with no
 * bundle have nothing worth a dedicated tab.
 */
export function isHukumEligible(subscription: HukumSubscription): boolean {
  return (
...
export function selectSubscription(
  ...
  teams: readonly HukumTeamSubscription[],
): HukumSubscription | null {
...
export function creditBreakdown(
  subscription: HukumSubscription,
): CreditBreakdown {
...
/**
 * The popover Hukum block's display state - the same cold/error/degraded shape
 ...
 */
export type HukumSubscriptionState =
  | { readonly kind: "cold" }
  | { readonly kind: "error" }
  | { readonly kind: "empty" }
  | {
...
export function resolveHukumSubscriptionState(args: {
  readonly isPending: boolean;
  readonly isError: boolean;
  readonly subscription: HukumSubscription | null;
}): HukumSubscriptionState {

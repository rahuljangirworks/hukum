...
function teamSub(teamId: string): HukumTeamSubscription {
  return {
    ...baseSubscription(),
    teamID: teamId,
...
function userWith(
  userSubscription: HukumUserSubscription,
  teams: HukumTeamSubscription[],
): AuthenticatedUser {
  return {
    user: {
      id: "u1",
...
describe("isHukumEligible", () => {
  it("is eligible on a paid plan", () => {
    expect(isHukumEligible(userSub({ status: "PRO_V3" }))).toBe(true);
  });

  it("is eligible on a free plan that holds an active bundle", () => {
    expect(
      isHukumEligible(userSub({ status: "FREE", hasActiveBundle: true })),
    ).toBe(true);
  });

  it("is not eligible on a free, unbundled plan", () => {
    expect(isHukumEligible(userSub({ status: "FREE" }))).toBe(false);
    expect(
      isHukumEligible(userSub({ status: "FREE", hasActiveBundle: false })),
    ).toBe(false);
  });
});
...
describe("resolveHukumSubscriptionState", () => {

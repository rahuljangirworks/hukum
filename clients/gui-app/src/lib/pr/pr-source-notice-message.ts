...
export function prSourceNoticeMessageFor(
  ...
  // user - one is GitHub refusing, the other is Hukum failing to reach it -
  ...
  switch (notice.kind) {
    ...
    case "backing-off":
      return countdown === null
        ? `Could not reach GitHub. ${what} are not refreshing; Hukum keeps retrying in the background.`

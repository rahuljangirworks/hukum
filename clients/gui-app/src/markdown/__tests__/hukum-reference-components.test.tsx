...
// `HukumMarkdown` bind to the mocked modules.
import { HukumMarkdown } from "@/markdown/hukum-markdown";
import { HukumSpecReference } from "@/markdown/components/hukum-spec-reference";
import { HukumTicketReference } from "@/markdown/components/hukum-ticket-reference";
import { HukumChatReference } from "@/markdown/components/hukum-chat-reference";
import { HukumEpicReference } from "@/markdown/components/hukum-epic-reference";
...
describe("legacy hukum-* reference components", () => {
  ...
  it("same-epic chat opens a chat tile as a preview", () => {
    render(
      <HukumChatReference data-epic-id={OPEN_EPIC_ID} data-chat-id="chat-1">
        Chat One
      </HukumChatReference>,
    );
  ...
  it("cross-epic chat reuses focusArtifactId (no focusChatId)", () => {
    render(
      <HukumChatReference data-epic-id="epic-other" data-chat-id="chat-42">
        Chat Forty-Two
      </HukumChatReference>,
    );
  ...
  it("hukum-epic focuses the epic with no artifact id", () => {
    render(
      <HukumEpicReference data-epic-id="epic-other">
        Some Epic
      </HukumEpicReference>,
    );
  ...
  it("renders plain text when the embedded id is missing", () => {
    render(
      <HukumSpecReference data-epic-id={OPEN_EPIC_ID}>
        Bare
      </HukumSpecReference>,
    );
  ...
  it("renders plain text when the epic id is missing", () => {
    render(
      <HukumChatReference data-chat-id="chat-1">
        No Epic
      </HukumChatReference>,
    );
  ...
  it("renders plain text when there is no open-epic session context", () => {
    mockHandle = null;
    render(
      <HukumSpecReference data-epic-id={OPEN_EPIC_ID} data-spec-id="spec-1">
        Detached Spec
      </HukumSpecReference>,
    );
...
describe("DEFAULT_COMPONENTS wires the legacy reference tags", () => {
  it("renders a clickable chip for a same-epic <hukum-spec> tag", () => {
    renderMarkdown(
      `<hukum-spec epicId="${OPEN_EPIC_ID}" specId="spec-1" title="Spec One">Spec One</hukum-spec>`,
    );

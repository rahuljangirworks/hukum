const HUKUM_NEXT_STEPS_OPEN_TAG = "<HUKUM_NEXT_STEPS>";
const HUKUM_NEXT_STEPS_CLOSE_TAG = "</HUKUM_NEXT_STEPS>";

export interface HukumNextStepOption {
  readonly id: string;
  readonly prompt: string;
}

export type HukumNextStepsPart =
  | {
  ...
  | {
      ...
      readonly options: ReadonlyArray<HukumNextStepOption>;
...
const CLOSE_LINE_PATTERN = /^[\t ]*<\/HUKUM_NEXT_STEPS>[\t ]*$/;
const OPEN_AT_LINE_START_PATTERN = /^[\t ]*<HUKUM_NEXT_STEPS>[\t ]*/;
...
const completedParseCache = new Map<
  string,
  ReadonlyArray<HukumNextStepsPart>
>();

export function parseHukumNextStepsMarkdown(
  markdown: string,
  isStreaming: boolean,
): ReadonlyArray<HukumNextStepsPart> {
  if (!markdown.includes(HUKUM_NEXT_STEPS_OPEN_TAG)) {
    return [
      {
        kind: "markdown",
        id: "markdown:0",
  ...
  if (!isStreaming && markdown.length <= MAX_COMPLETED_PARSE_CACHE_CHARS) {
    ...
    const parsed = parseHukumNextStepsMarkdownWithTags(markdown, false);
    completedParseCache.set(markdown, parsed);
    trimCompletedParseCache();
    return parsed;
  }

  return parseHukumNextStepsMarkdownWithTags(markdown, isStreaming);
}

function parseHukumNextStepsMarkdownWithTags(
  markdown: string,
  isStreaming: boolean,
): ReadonlyArray<HukumNextStepsPart> {
  ...
  const parts: HukumNextStepsPart[] = [];
  let cursor = 0;
...
export function repairHukumNextStepsMarkdown(markdown: string): string {
  if (!markdown.includes(HUKUM_NEXT_STEPS_OPEN_TAG)) return markdown;
  const blocks = extractNextStepsBlocks(markdown, true);
  if (blocks.length === 0) return markdown;
  const last = blocks.at(-1);
  if (last === undefined || last.complete) return markdown;
  return `${markdown.trimEnd()}\n${HUKUM_NEXT_STEPS_CLOSE_TAG}`;
}
...
function parseNextStepsBlock(content: string): {
  ...
  const options: HukumNextStepOption[] = [];

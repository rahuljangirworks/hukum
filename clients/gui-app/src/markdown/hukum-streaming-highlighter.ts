...
/**
 * Theme-aware StreamingHighlighter over Hukum's multi-preset Shiki core and
 ...
 * workspace surface.
 */
class HukumStreamingHighlighter implements StreamingHighlighter {
...
const hukumStreamingHighlighter = new HukumStreamingHighlighter();

/** Shared adapter for StreamingMarkdown and product CodeBlock chrome. */
export function getHukumStreamingHighlighter(): StreamingHighlighter {
  return hukumStreamingHighlighter;
}

/** Drop singleton readiness state between tests that mock the Shiki boot path. */
export function resetHukumStreamingHighlighterForTests(): void {
  hukumStreamingHighlighter.resetForTests();
}

...
/**
 * The click handler a legacy `<hukum-*>` reference component runs, or `null`
 ...
 */
export type HukumReferenceOpenHandler =
  ((event: MouseEvent<HTMLElement>) => void) | null;

/**
 * What `useHukumReferenceOpenHandler` resolves in a single render pass:
 *
 ...
 */
export interface HukumReferenceOpenState {
  readonly onOpen: HukumReferenceOpenHandler;
 ...
 *   `focusArtifactId` for a node (D1); `hukum-epic` carries `undefined`.
 */
type OpenTarget =
  | { readonly kind: "none" }
  | {
...
/**
 * Builds the open/navigate handler shared by every legacy `<hukum-*>`
 ...
 *   `<hukum-epic>`, which focuses the epic without an artifact.
 ...
 *   re-click after closing the tab re-opens it. `<hukum-epic>` passes no
 ...
 * text.
 */
export function useHukumReferenceOpenHandler(input: {
   ...
   * `false` (`hukum-epic`), the reference focuses the epic with no node.
   */
  readonly requiresNode: boolean;
}): HukumReferenceOpenState {
  ...
  const handler = useCallback(
    (event: MouseEvent<HTMLElement>): void => {
      ...
      switch (target.kind) {
        ...
        case "navigate":
          // `focusArtifactId` opens chat tiles too (D1); `hukum-epic` passes
...
function resolveOpenTarget(input: {
  ...
  // CROSS epic (or same-epic epic-only focus): navigate + focus. `hukum-epic`

/**
 * Presentational, query-free views for the Hukum subscription surface, shared
 * by the Settings › Providers › Hukum card (`hukum-subscription-section.tsx`)
 * and the header rate-limit popover's "Hukum" tab (`rate-limit-popover.tsx`) -
 ...
 * disagree.
 *
 * `HukumSubscriptionView` is the shared body (credit/rate-limit breakdown -
 ...
 * variant-parameterized: the Hukum body has no per-model / extra-window rows
 ...
 * Codex/Claude windows use, so Hukum's own bars read identically
...
import { useRefreshRateLimitUsageOnHukumTurn } from "@/hooks/host/use-refresh-rate-limit-usage-on-hukum-turn";
import {
  PERSONAL_VALUE,
  TEAM_VALUE_PREFIX,
  creditBreakdown,
 ...
 * popover's Hukum detail tab. Renders nothing when the user has no teams (the
 * only choice is Personal, so a one-option select is noise) - the caller can
 * always render it unconditionally.
 */
export function HukumAccountSelect({
  teams,
  value,
  onValueChange,
}: {
 ...
 */
export function HukumSubscriptionView({
  subscription,
  accountContext,
}: {
  readonly subscription: HukumSubscription;
...
function RateLimitView({
  ...
  // Keep the bar live: a Hukum turn finishing while this is on screen
  // re-fetches usage. Only mounted here, so it costs nothing elsewhere.
  useRefreshRateLimitUsageOnHukumTurn(accountContext);
 ...
 * in `provider-rate-limit-views.tsx`), Hukum's own bars (`CreditMeterRow`
 ...
 * via the shared `MeterRow` shell, so Hukum's own bars read identically to

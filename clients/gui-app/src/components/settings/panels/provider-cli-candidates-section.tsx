...
import { useProvidersUpdateBundled } from "@/hooks/providers/use-providers-update-bundled-mutation";
 ...
 * `baseBinaryName`/`providerSubdir` map hukum and openrouter onto opencode).
 * `null` for a provider that owns its binary.
 *
 * Exhaustive rather than the `hukum || openrouter` id test it replaces:
...
/**
 * Where to send a user whose provider ships no Hukum-managed binary and has
    ...
    <>
      ...
      */}
      ...
      {/*
        ...
        paired with the exact build Hukum ships, so a version found on PATH is
 ...
 * (their SDKs spawn their own copies, so Hukum vendors neither) and would
...
function CliBinaryMissingNotice({
  ...
  return (
    <div className="rounded-lg border border-border/60 bg-muted/10 p-3 text-ui-sm text-muted-foreground">
      <p>
        No {providerLabel} CLI was found on this machine, and Hukum ships no
 ...
 * binary the user could pick. Both of them spawn the Hukum-resolved binary
...
export function ProviderCliCandidatesSection({
  ...
  const updateBundled = useProvidersUpdateBundled();
            ...
            <CandidateRow
              ...
              onUpdateBundled={
                candidate.kind === "bundled"
                  ? () => updateBundled.mutate({ providerId })
                  : undefined
              }
              updatingBundled={updateBundled.isPending}
            />
          ))}
          {adding ? (
...
function CandidateRow({
  ...
  readonly onUpdateBundled?: () => void;
  readonly updatingBundled?: boolean;
}): ReactNode {
  ...
  return (
    ...
    >
      ...
      <span className="flex items-center justify-center py-2.5">
        ...
        ) : isBundled && onUpdateBundled ? (
          <Button
            type="button"
            variant="ghost"
            size="icon"
            disabled={busy || updatingBundled}
            onClick={onUpdateBundled}
            className="size-6 text-muted-foreground transition-colors hover:text-foreground disabled:opacity-50"
            title="Update bundled provider"
          >
            {updatingBundled ? (
              <MutedAgentSpinner />
            ) : (

...
describe("OnboardingPage", () => {
  ...
  it("shows the continue button (not 'Enter Hukum') on the first act", () => {
    renderPage({ replay: false });
  ...
  it("wires onboarding footer links to the website destinations", () => {
    ...
    const expectedLinks = [
      ["Features", hukumInfo.mainWebsiteFeatures],
      ["Enterprise", hukumInfo.mainWebsiteEnterprise],
      ["Support", hukumInfo.mainWebsiteContactUs],
    ] as const;
  ...
  it("falls back to browser navigation when the runner host cannot open a footer link", async () => {
    ...
    await waitFor(() => {
      expect(windowOpenMock).toHaveBeenCalledWith(
        hukumInfo.mainWebsiteContactUs,
        "_blank",
        "noopener,noreferrer",
      );
    });
  ...
  it("keeps onboarding navigation available while provider discovery settles", async () => {
    guideQueryState = {
      data: {
        content: null,
        generatedDefaultContent: "hukum guide",

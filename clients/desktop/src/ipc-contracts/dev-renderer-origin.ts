...
export const HUKUM_DESKTOP_DEV_URL_ENV = "HUKUM_DESKTOP_DEV_URL";
...
export function devRendererUrlFromEnv(env: NodeJS.ProcessEnv): string {
  const raw = env[HUKUM_DESKTOP_DEV_URL_ENV] ?? DEFAULT_DEV_RENDERER_URL;
...
export function normalizeDevRendererUrl(value: string): string {
  ...
  try {
    ...
    throw new Error(`${HUKUM_DESKTOP_DEV_URL_ENV} must be a valid URL`);
  }
  if (url.protocol !== "http:") {
    throw new Error(`${HUKUM_DESKTOP_DEV_URL_ENV} must use http`);
  }
  if (!ALLOWED_DEV_RENDERER_HOSTS.has(url.hostname)) {
    throw new Error(`${HUKUM_DESKTOP_DEV_URL_ENV} must use a loopback host`);
  }
  if (url.port.length === 0) {
    throw new Error(`${HUKUM_DESKTOP_DEV_URL_ENV} must include a port`);
  }
  if (url.username.length > 0 || url.password.length > 0) {
    throw new Error(
      `${HUKUM_DESKTOP_DEV_URL_ENV} must not include credentials`,
    );
  }
  if (url.pathname !== "/" || url.search.length > 0 || url.hash.length > 0) {
    throw new Error(`${HUKUM_DESKTOP_DEV_URL_ENV} must be an origin URL`);
  }
  return url.origin;
}

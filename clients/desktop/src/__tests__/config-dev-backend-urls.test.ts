...
describe("desktop config dev backend URL overrides", () => {
  it("keeps the committed production URLs when the env vars are unset", async () => {
    ...
    expect(config.authnBaseUrl).toBe("https://authn.hukum.ai");
    expect(config.cloudUiBaseUrl).toBe("https://platform.hukum.ai");
    expect(config.relayBaseUrl).toBe("wss://relay.hukum.ai/attach");
  });

  it("honors loopback overrides and keeps everything else baked", async () => {
    vi.stubEnv("HUKUM_DEV_AUTHN_BASE_URL", "http://localhost:21001");
    vi.stubEnv("HUKUM_DEV_CLOUD_UI_BASE_URL", "http://localhost:21003");
    vi.stubEnv("HUKUM_DEV_RELAY_BASE_URL", "ws://localhost:8787/attach");
  ...
  it("throws at module init on a non-loopback override", async () => {
    vi.stubEnv("HUKUM_DEV_AUTHN_BASE_URL", "http://evil.example.com:80");
    await expect(import("../config")).rejects.toThrow(/loopback/);
  });

  it("throws at module init on a non-ws relay override", async () => {
    vi.stubEnv("HUKUM_DEV_RELAY_BASE_URL", "wss://localhost:8787/attach");

 ...
 * `hukum-cli/src/commands/`). `runHukumCliJsonWithInvocation` used to
 ...
 */
export interface HukumCliErrorInit {
...
export class HukumCliError extends Error {
  readonly code: string | null;
  readonly details: unknown;
  readonly exitCode: number | null;
  readonly stderrTail: string;

  constructor(init: HukumCliErrorInit, legacyMessage: null);
  constructor(code: string, message: string);
  constructor(
    initOrCode: HukumCliErrorInit | string,
    legacyMessage: string | null,
  ) {
    if (typeof initOrCode === "string") {
      super(typeof legacyMessage === "string" ? legacyMessage : initOrCode);
      this.name = "HukumCliError";
 ...
 * shape on failure).
 *
 * The shared runner in `hukum-cli/src/runner/output.ts` is the
...
/**
 * Locates the `hukum` CLI command for subprocess invocation.
 *
 * Resolution is identical in packaged and unpackaged builds (Tech Plan
 * Decision 6):
 *
 *   1. CLI manifest (`~/.hukum/cli/manifest.json`) - package-manager
 *      or Desktop-staged CLI is authoritative.
 *   2. PATH fallback - `hukum` (or `hukum.exe` on Windows) on PATH.
 ...
 * args after.
 */
export interface HukumCliInvocation {
  readonly command: string;
  readonly args: readonly string[];
}

export async function resolveHukumCliInvocation(): Promise<HukumCliInvocation> {
  ...
  throw new Error(
    `hukum CLI: no CLI found via manifest, PATH, or bundled resources (looked for ${cliBinaryName()}). Packaged builds bundle the CLI under resources/cli; \`make dev-desktop\` stages the dev CLI wrapper.`,
  );
}
 ...
 * self-management."). Skips the manifest/PATH steps `resolveHukumCliInvocation`
 ...
 */
export async function resolveBundledHukumCliInvocation(): Promise<HukumCliInvocation> {
  const bundled = await resolveBundledCliPath();
  if (bundled !== null) {
    return { command: bundled, args: [] };
  }
  throw new Error(
    `hukum CLI: no bundled CLI found (looked for ${cliBinaryName()} under app resources). This is a broken install - run \`hukum host doctor\` or reinstall Hukum.`,
  );
}

export interface RunHukumCliOptions {
  /**
...
export interface HukumCliResult {
  readonly stdout: string;
  readonly stderr: string;
}

export async function runHukumCli(
  opts: RunHukumCliOptions,
): Promise<HukumCliResult> {
  const inv = await resolveHukumCliInvocation();
  return runHukumCliWithInvocation(inv, opts);
}

/**
 * Same as `runHukumCli`, but the caller supplies an already-resolved
 * `HukumCliInvocation` instead of letting this module resolve one via
 * `resolveHukumCliInvocation()`. Extracted so `runBundledHukumCliJson`
 ...
 * manifest/PATH steps.
 */
async function runHukumCliWithInvocation(
  inv: HukumCliInvocation,
  opts: RunHukumCliOptions,
): Promise<HukumCliResult> {
    ...
    execFile(
      ...
      (err, stdout, stderr) => {
        if (err !== null) {
          ...
          log.warn("[hukum-cli] subprocess failed", {
            command: inv.command,
            args: allArgs,
          ...
          // the missing case made `runHukumCliJson`'s envelope
 ...
 * and are invoked through `runHukumCliJson`.
 *
 ...
 */
export async function runHukumCliPlainJson<T>(
  args: readonly string[],
): Promise<T> {
  const augmented = ensureJsonFlag(args);
  let result: HukumCliResult;
  try {
    result = await runHukumCli({
      args: augmented,
      maxBuffer: 1024 * 1024,
      timeoutMs: 10_000,
    });
    ...
    if (salvaged !== null) {
      log.warn("[hukum-cli] non-zero exit after complete plain JSON", {
        args: augmented,
  ...
  if (trimmed.length === 0) {
    throw new HukumCliError(
      {
        message: `hukum-cli emitted no stdout for: ${augmented.join(" ")}`,
        code: null,
        details: null,
        exitCode: 0,
  ...
  try {
    ...
    throw new HukumCliError(
      {
        message: `hukum-cli stdout was not valid JSON for: ${augmented.join(" ")} (${reason})`,
        code: null,
        details: null,
        exitCode: 0,
        stderrTail: result.stderr.slice(-2048),
 ...
 *   - drops `progress` records (use `streamHukumCliJson` to receive them)
 *   - on `status: "ok"` resolves with the inner `data` field
 *   - on `status: "error"` rejects with a `HukumCliError` carrying the
 ...
 * (e.g. spawn error / crash) we still reject with a `HukumCliError`
 * so Desktop projectors never see a half-formed payload.
 */
export async function runHukumCliJson<T>(
  args: readonly string[],
): Promise<T> {
  const inv = await resolveHukumCliInvocation();
  return runHukumCliJsonWithInvocation(inv, args);
}

/**
 * Bundled-only counterpart to `runHukumCliJson` (D7: `HostController`
 * host operations always invoke the desktop-bundled, version-matched CLI -
 * never the discovered manifest/PATH CLI `runHukumCliJson` resolves).
 * Same envelope contract; only the invocation resolution differs.
 */
export async function runBundledHukumCliJson<T>(
  args: readonly string[],
): Promise<T> {
  const inv = await resolveBundledHukumCliInvocation();
  return runHukumCliJsonWithInvocation(inv, args);
}

async function runHukumCliJsonWithInvocation<T>(
  inv: HukumCliInvocation,
  args: readonly string[],
): Promise<T> {
  const augmented = ensureJsonFlag(args);
  let result: HukumCliResult;
  try {
    result = await runHukumCliWithInvocation(inv, {
      args: augmented,
      maxBuffer: 1024 * 1024,
    ...
    // `hukum host doctor --json` is the canonical case: it emits a
    ...
    if (fromEnvelope instanceof HukumCliError) {
      throw fromEnvelope;
    }
  ...
  if (envelope === null) {
    throw new HukumCliError(
      {
        message: `hukum-cli emitted no terminal result line for: ${augmented.join(" ")}`,
        code: null,
        details: null,
        exitCode: 0,
        stderrTail: result.stderr.slice(-2048),
  ...
  if (envelope instanceof HukumCliError) {
    throw envelope;
  }
  return envelope as T;
}

export interface StreamHukumCliOptions {
  readonly args: readonly string[];
  ...
  // any connection slower than <archive size> / <cap> (hukum#585/#589):
...
export interface StreamHukumCliResult<T> {
  readonly data: T;
}
 ...
 */
export async function streamHukumCliJson<T>(
  opts: StreamHukumCliOptions,
): Promise<StreamHukumCliResult<T>> {
  const inv = await resolveHukumCliInvocation();
  return streamHukumCliJsonWithInvocation(inv, opts);
}

/**
 * Bundled-only counterpart to `streamHukumCliJson` (D7: every CLI
 ...
 */
export async function streamBundledHukumCliJson<T>(
  opts: StreamHukumCliOptions,
): Promise<StreamHukumCliResult<T>> {
  const inv = await resolveBundledHukumCliInvocation();
  return streamHukumCliJsonWithInvocation(inv, opts);
...
async function streamHukumCliJsonWithInvocation<T>(
  inv: HukumCliInvocation,
  opts: StreamHukumCliOptions,
): Promise<StreamHukumCliResult<T>> {
  const augmentedArgs = ensureJsonFlag(opts.args);
  const allArgs = [...inv.args, ...augmentedArgs];
  return new Promise<StreamHukumCliResult<T>>((resolve, reject) => {
    ...
    let terminalError: HukumCliError | null = null;
    let abortError: HukumCliError | null = null;
    let timeoutError: HukumCliError | null = null;
    let settled = false;
    ...
    const onIdleTimeout = (): void => {
      ...
      // cancellation, so Remove Hukum's download-drain cannot launch an
      // uninstall while the child is still exiting.
      timeoutError = new HukumCliError(
        {
          message: `hukum-cli produced no output for ${opts.idleTimeoutMs}ms (${augmentedArgs.join(" ")})`,
          code: null,
          details: null,
          exitCode: null,
          stderrTail,
    ...
    // ran to completion regardless, so Remove Hukum's cancellation was
    ...
    const onAbort = (): void => {
      if (settled || abortError !== null || timeoutError !== null) return;
      clearIdleTimer();
      abortError = new HukumCliError(
        {
          message: `hukum-cli aborted: ${augmentedArgs.join(" ")}`,
          code: null,
          details: null,
          exitCode: null,
          stderrTail,
    ...
    child.stdout.on("data", (chunk: string) => {
      ...
      while (newlineIndex !== -1) {
        ...
        try {
          ...
          log.warn("[hukum-cli] ignored non-JSON stdout line", {
        ...
        terminalError = new HukumCliError(
          {
    ...
    child.on("close", (exitCode, signal) => {
      ...
      if (signal !== null) {
        reject(
          new HukumCliError(
            {
              message: `hukum-cli was killed by ${signal}: ${augmentedArgs.join(" ")}`,
              code: null,
              details: null,
              exitCode,
              stderrTail,
      ...
      // terminator on the very next statement (hukum-cli's runner.ts), so
      ...
      // own teardown, not the work. `hukum host doctor` is the intentional
      ...
      // fails below, exactly as before. `runHukumCliJsonWithInvocation`
      // has made the same call on the non-streaming path since it shipped.
      if (sawTerminalOk) {
        if (typeof exitCode === "number" && exitCode !== 0) {
          log.warn("[hukum-cli] non-zero exit after a successful result", {
            args: augmentedArgs,
            exitCode,
      ...
      if (typeof exitCode === "number" && exitCode !== 0) {
        reject(
          new HukumCliError(
            {
              message: `hukum-cli exited with code ${exitCode}: ${augmentedArgs.join(" ")}`,
              code: null,
              details: null,
              exitCode,
              stderrTail,
      ...
      reject(
        new HukumCliError(
          {
            message: `hukum-cli emitted no terminal result for: ${augmentedArgs.join(" ")}`,
            code: null,
            details: null,
            exitCode,
            stderrTail,
 ...
 *   - a `HukumCliError` on an error envelope
 *   - `null` when no terminal line is present at all
 *
 * Used by `runHukumCliJson` so query commands get the same envelope
...
function extractTerminalEnvelope(
  ...
  for (const line of lines) {
    ...
    try {
      ...
      log.warn("[hukum-cli] ignored non-JSON terminal stdout line", {
        lineLength: line.length,
      });
      continue;
    }

...
// `hukum-cli-envelope.test.ts`: a REAL subprocess, REAL pipes and REAL
// timers, so this proves the wiring end to end rather than the timer
// arithmetic in isolation.
//
// The regression it guards is hukum#585/#589: `CLI_STREAM_TIMEOUT_MS` was
...
const scriptDir = mkdtempSync(join(tmpdir(), "hukum-cli-idle-itest-"));
...
import { streamHukumCliJson } from "../hukum-cli";

// Stands in for `hukum host download --json`: emits NDJSON progress on a
...
beforeAll(() => {
  process.env.HUKUM_TEST_FAKE_CLI = "1";
});
...
describe("streamHukumCliJson idle budget against a real subprocess", () => {

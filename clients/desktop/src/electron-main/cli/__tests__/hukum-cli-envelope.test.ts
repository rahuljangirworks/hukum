...
// passed `runHukumCliJson`'s parsed-JSON return straight to the
// host-management projector functions, but the shared runner in
// `hukum-cli/src/runner/output.ts` wraps every terminal payload in a
...
//   - error envelopes reject with `HukumCliError` carrying the
...
// `resolveHukumCliInvocation` walks the real discovery chain (manifest →
...
// `runHukumCli*` envelope/error contract without coupling to the dev
// CLI wrapper being staged at `~/.hukum/cli/dev/bin/hukum`. The
...
vi.mock("../cli-discovery", async (importOriginal) => {
  ...
  return {
    ...
    discoverCli: async () => ({
      ...
      binaryPath: "/tmp/hukum-test/discovered-cli/hukum",
    }),
    resolveBundledCliPath: async () => "/tmp/hukum-test/bundled-cli/hukum",
  };
});
...
describe("runHukumCliJson unwraps result envelopes", () => {
  it("returns the inner data payload from {type:result, status:ok} envelopes", async () => {
    const doctorIssues = [
      {
        ...
        terminalCommand: "hukum host restart",
    ...
    const { runHukumCliJson } = await import("../hukum-cli");
    const result = await runHukumCliJson<{ issues: typeof doctorIssues }>([
      "host",
      "doctor",
    ]);
  ...
  it("appends --json when the caller forgot it", async () => {
    ...
    const { runHukumCliJson } = await import("../hukum-cli");
    await runHukumCliJson(["host", "uninstall"]);
    expect(setup.capturedArgs[0]).toContain("--json");
  });

  it("rejects with HukumCliError carrying the CLI error code on {status:error} envelopes", async () => {
    const envelope = {
      type: "result",
      status: "error",
      error: {
        ...
        details: { livePath: "/usr/local/bin/hukum" },
      },
      timestamp: "2026-05-15T00:00:00Z",
    };
    configureExecFile({
    ...
    const { runHukumCliJson, HukumCliError } =
      await import("../hukum-cli");
    await expect(runHukumCliJson(["cli", "upgrade"])).rejects.toMatchObject({
      code: "E_CLI_UPGRADE_REPLACE_FAILED",
      message: expect.stringContaining("replace failed"),
    });
    // Stable construction even on the error path - the stderr tail
    // makes it into the surfaced HukumCliError instance.
    let thrown: unknown = null;
    try {
      await runHukumCliJson(["cli", "upgrade"]);
    } catch (err) {
      thrown = err;
    }
    expect(thrown).toBeInstanceOf(HukumCliError);
  });
  ...
  it("extracts the error envelope even when execFile rejects with an Error that lacks .stdout/.stderr (Electron/Node decoration gap)", async () => {
    ...
    // runHukumCliJson's catch had no .stdout / .stderr attached, so
    ...
    // never made it to the user. `runHukumCli` now (re-)attaches both
    ...
    execFileImpl = (_cmd, _args, _opts, callback) => {
      ...
      const bareErr = new Error("Command failed: /tmp/hukum ...");
    ...
    let thrown: unknown = null;
    try {
      await runHukumCliJson(["host", "available"]);
    } catch (err) {
      thrown = err;
    }
    expect(thrown).toBeInstanceOf(HukumCliError);
  ...
  it("ignores progress events before the terminal result line", async () => {
    ...
    const { runHukumCliJson } = await import("../hukum-cli");
    const result = await runHukumCliJson<{ final: boolean; version: string }>(
      ["host", "available"],
    );
...
describe("streamHukumCliJson resolves data, fans progress, and converts error envelopes", () => {
  it("V10: bundled wrappers invoke the bundled CLI instead of the discovered CLI", async () => {
    ...
    const { runBundledHukumCliJson, streamBundledHukumCliJson } =
      await import("../hukum-cli");

    await runBundledHukumCliJson<{ version: string }>(["host", "status"]);
    await streamBundledHukumCliJson<{ version: string }>({
    ...
    expect(runSetup.capturedCommands).toEqual([
      "/tmp/hukum-test/bundled-cli/hukum",
    ]);
    expect(streamCommand).toBe("/tmp/hukum-test/bundled-cli/hukum");
  });
  ...
  it("forces --json onto args and emits each progress event before resolving with unwrapped data", async () => {
    ...
    const { streamHukumCliJson } = await import("../hukum-cli");
    const progressEvents: Array<{ stage: string; percent: number | null }> = [];
    const result = await streamHukumCliJson<{ version: string }>({
  ...
  it("rejects with HukumCliError when the CLI emits a {status:error} terminal envelope", async () => {
    ...
    const { streamHukumCliJson, HukumCliError } =
      await import("../hukum-cli");
    let thrown: unknown = null;
    try {
      await streamHukumCliJson<unknown>({
    ...
    expect(thrown).toBeInstanceOf(HukumCliError);
    if (thrown instanceof HukumCliError) {
...
//
// `runHukumCliJson*` has always made the opposite call on the non-streaming
...
describe("streamHukumCliJson trusts a completed terminal result over the exit code", () => {
...
// Fixup C4: `streamBundledHukumCliJson`'s only cancellable caller
...
// regardless, so `removeHukum`'s "abort the in-flight download" claim was
...
describe("streamHukumCliJson kills the subprocess when its signal aborts", () => {
  it("kills the still-running child but waits for close before rejecting", async () => {
    ...
    const promise = streamHukumCliJson<unknown>({
      args: ["host", "download", "--automatic"],
      onEvent: () => undefined,
      env: null,
      idleTimeoutMs: 5_000,
...
describe("streamHukumCliJson reports an external kill by its signal", () => {
...
describe("streamHukumCliJson timeout waits for the child close", () => {
  ...
  // hukum#585/#589: this budget is inactivity, not wall-clock. A 700MB
...
describe("runHukumCliPlainJson preserves legacy plain-JSON output", () => {
  it("parses a single plain JSON document (no NDJSON envelope) for `host status --json`", async () => {
    ...
    // it through `runHukumCliJson` (envelope-only) would reject with
    // "emitted no terminal result line"; `runHukumCliPlainJson`
    ...
    const plain = {
      ...
      bootstrapMarkers: [],
      bootstrapLogPath: "/Users/test/.hukum/host/bootstrap.log",
      bootstrapLogTail: "",
    };
    configureExecFile({
    ...
    const { runHukumCliPlainJson } = await import("../hukum-cli");
    const result = await runHukumCliPlainJson<typeof plain>([
      "host",
      "status",
      "--json",
    ]);
  ...
  it("ensures --json is appended even when callers forget it", async () => {
    ...
    const { runHukumCliPlainJson } = await import("../hukum-cli");
    await runHukumCliPlainJson(["config", "shell", "get"]);
    expect(setup.capturedArgs[0]).toContain("--json");
  });

  it("rejects with HukumCliError when stdout is not valid JSON", async () => {
    configureExecFile({
      stdout: "this is not json\n",
      stderr: "",
      exitCode: 0,
    });
    const { runHukumCliPlainJson, HukumCliError } =
      await import("../hukum-cli");
    let thrown: unknown = null;
    try {
      await runHukumCliPlainJson(["host", "status", "--json"]);
    } catch (err) {
      thrown = err;
    }
    expect(thrown).toBeInstanceOf(HukumCliError);
  ...
  it("rejects with HukumCliError on non-zero exit with stderr tail attached", async () => {
    configureExecFile({
      stdout: "",
      stderr: "boom\n",
      exitCode: 2,
    ...
    let thrown: unknown = null;
    try {
      await runHukumCliPlainJson(["config", "env", "list", "--json"]);
    } catch (err) {
      thrown = err;
    }
    expect(thrown).toBeInstanceOf(HukumCliError);
...
describe("runHukumCliJson preserves successful envelopes on non-zero exit", () => {
  // `hukum host doctor --json` emits a fully-formed
  ...
  it("resolves with unwrapped data when stdout has a success envelope but exit code is non-zero (Doctor case)", async () => {
    const issues = [
      {
        ...
        message: "Start the host to use Hukum.",
        fixAction: "host-start",
        terminalCommand: "hukum host start",
        details: null,
      },
      {
        code: "CLI_UPGRADE_PENDING",
    ...
    const { runHukumCliJson } = await import("../hukum-cli");
    const result = await runHukumCliJson<{ issues: typeof issues }>([
      "host",
      "doctor",
    ]);
    ...
    expect(pending?.terminalCommand).toBe("hukum host restart");
  });

  it("still rejects with HukumCliError when non-zero exit emits an error terminal envelope on stdout", async () => {
    ...
    let thrown: unknown = null;
    try {
      await runHukumCliJson(["host", "install", "latest"]);
    } catch (err) {
      thrown = err;
    }
    expect(thrown).toBeInstanceOf(HukumCliError);
  ...
  it("rejects with HukumCliError carrying exit code + stderr tail when non-zero exit emits no parseable envelope", async () => {
    configureExecFile({
    ...
    let thrown: unknown = null;
    try {
      await runHukumCliJson(["host", "doctor"]);
    } catch (err) {
      thrown = err;
    }
    expect(thrown).toBeInstanceOf(HukumCliError);
...
describe("runHukumCliJson timeout must exceed the CLI's own lock wait (fixup A8)", () => {
  it("passes execFile a timeout that exceeds the CLI's 30s cli-lock wait, not merely matches or falls short of it", async () => {
    ...
    const { runHukumCliJson } = await import("../hukum-cli");
    await runHukumCliJson(["host", "service", "uninstall"]);
...
describe("CLI_UPGRADE_PENDING preservation through Desktop projection", () => {
  it("survives projectDoctorReport after envelope unwrapping", async () => {
    ...
    // `runHukumCliJson` which unwraps `data` first.
    const pendingIssue = {
      code: "CLI_UPGRADE_PENDING",
      severity: "warning" as const,
      title: "CLI upgrade staged",
    ...
    const { runHukumCliJson } = await import("../hukum-cli");
    const data = await runHukumCliJson<{
    ...
    expect(issue?.terminalCommand).toBe("hukum host restart");
  });
});

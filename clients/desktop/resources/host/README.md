...
> install/update/uninstall is owned by the **Hukum CLI** and backed by the
...
`~/.hukum/host/pid.json` (prod channel) and `~/.hukum/host/dev/pid.json`
...
directory; `hukum host install latest` (invoked by the Setup splash at first
...
```bash
hukum host install --from /absolute/path/to/host.tgz
```
...
## What about `resolveHostBinaryPath` / `HUKUM_HOST_BINARY`?
